...
static bool __dupDescriptor(int fd, int fdn, int *backup_fd, int *backup_fdn) {
#if __WINDOWS__
...
RZ_API int rz_cons_pipe_open(const char *file, int fdn, int *backup_fd, int *backup_fdn ,int append) {
	if (fdn < 1) {
		return -1;
	}
	...
	(*backup_fdn) = fdn;
	if (!__dupDescriptor(fd, fdn, backup_fd, backup_fdn)) {
...
RZ_API void rz_cons_pipe_close(int fd, int *backup_fd, int *backup_fdn) {
	if (fd != -1) {
		close(fd);
		if ((*backup_fd) != -1) {
