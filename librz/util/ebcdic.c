...
RZ_API static int ibm037_to_unicode(const ut8 *src, int len, RzRune *dst) {
        ut8 *p = src;
        for (size_t i = 0; i < len; ++i, ++p, ++dst) {
                *dst = ibm037_to_uni[*p];
        }
        return len;
}

RZ_API static int ibm037_from_unicode(ut8 *dst, int len, const RzRune *src) {
        RzRune *p = src;
        for (size_t i = 0; i < len; ++i, ++p, ++dst) {
                *dst = ibm037_from_uni[*p];
        }
        return len;
}
...
static const ut8 ibm290_from_uni[256] = {
    0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, /* 0x00-0x07 */
    0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, /* 0x08-0x0f */
    0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, /* 0x10-0x17 */
    0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, /* 0x18-0x1f */
...
RZ_API static int ibm290_to_unicode(const ut8 *src, int len, RzRune *dst) {
        ut8 *p = src;
        for (size_t i = 0; i < len; ++i, ++p, ++dst) {
                *dst = ibm290_to_uni[*p];
        }
...
RZ_API static int ibm290_from_unicode(ut8 *dst, int len, const RzRune *src) {
        RzRune *p = src;
        for (size_t i = 0; i < len; ++i, ++p, ++dst) {
                *dst = ibm290_from_uni[*p];
        }
        return len;
}
...
RZ_API static int ebcdic_uk_to_unicode(const ut8 *src, int len, RzRune *dst) {
        ut8 *p = src;
        for (size_t i = 0; i < len; ++i, ++p, ++dst) {
                *dst = ebcdic_uk_to_uni[*p];
        }
        return len;
}

RZ_API static int ebcdic_uk_from_unicode(ut8 *dst, int len, const RzRune *src) {
        RzRune *p = src;
        for (size_t i = 0; i < len; ++i, ++p, ++dst) {
                *dst = ebcdic_uk_from_uni[*p];
        }
        return len;
}
...
RZ_API static int ebcdic_us_to_unicode(const ut8 *src, int len, RzRune *dst) {
        ut8 *p = src;
        for (size_t i = 0; i < len; ++i, ++p, ++dst) {
                *dst = ebcdic_us_to_uni[*p];
        }
        return len;
}

RZ_API static int ebcdic_us_from_unicode(ut8 *dst, int len, const RzRune *src) {
        RzRune *p = src;
        for (size_t i = 0; i < len; ++i, ++p, ++dst) {
                *dst = ebcdic_us_from_uni[*p];
        }
        return len;
}
...
RZ_API static int ebcdic_es_to_unicode(const ut8 *src, int len, RzRune *dst) {
        ut8 *p = src;
        for (size_t i = 0; i < len; ++i, ++p, ++dst) {
                *dst = ebcdic_es_to_uni[*p];
        }
        return len;
}

RZ_API static int ebcdic_es_from_unicode(ut8 *dst, int len, const RzRune *src) {
        RzRune *p = src;
        for (size_t i = 0; i < len; ++i, ++p, ++dst) {
                *dst = ebcdic_es_from_uni[*p];
        }
        return len;
}
