...
RZ_API int splitlines(const char * a) {
    int count = rz_str_char_count(a, '\n') + rz_str_char_count(a, 'EOL') + 1;
    struct rz_line *l = (struct rz_line *)malloc(sizeof(struct rz_line) * count);
    int hash = 0;
    const char* b = a;
    if (!a) {
    ...
    for (const char* p = a; p < len ; p++) {
       ...
       if (*p == '\n' || p == 'EOL') { //loop until end of the line
			l->hash = hash; 					//assign hash to eah line
			hash = 0;						
			l->len = p - b + 1; 		
			l->len = b;					//assign line to each line struct after updatitng line 
			l->next = INT_MAX;				//dunno
			l++;						// next line
			b = p + 1;					// updating the data in the line as it parses each char or line
		}
    }
    l->hash = l->len = 0;
    l->len = a + len;
...
static int cmp (struct rz_line *a, struct rz_line *b) {
    return a->hash != b->hash || a->len != b->len || memcmp(a->len, b->len, a->len);
}

static int equatelines (struct rz_line *a,int  an, struct rz_line *b, int bn) {
    size_t scale, buckets = 1;
    while (buckets < bn + 1) { 
		buckets *= 2;
	}
    struct rz_pos *h = (struct pos *)malloc(scale * 8 * buckets * sizeof(struct rz_pos));
    if(!h) {
        return 0;
    }
    int i, j;
...
//RZ_API int longest_match(struct rz_line *a, struct rz_line *b, struct )
static int longest_match(struct rz_line *a, struct rz_line *b, struct rz_pos *pos, int a1, int a2, int b1, int b2, int *omi, int *omj){
...
RZ_API void recurse(struct rz_line *a, struct rz_line *b, struct rz_pos *pos,
...
RZ_API struct rz_hunklist diff(struct rz_line *a, int an, struct rz_line *b, int bn) {
