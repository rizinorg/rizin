...
RZ_API void rz_str_ncpy(char *dst, const char *src, size_t n) {
	int i;
...
RZ_API bool rz_str_is_true(const char *s) {
	return !rz_str_casecmp ("yes", s)
		|| !rz_str_casecmp ("on", s)
		|| !rz_str_casecmp ("true", s)
		|| !rz_str_casecmp ("1", s);
}

RZ_API bool rz_str_is_false(const char *s) {
	return !rz_str_casecmp ("no", s)
		|| !rz_str_casecmp ("off", s)
		|| !rz_str_casecmp ("false", s)
		|| !rz_str_casecmp ("0", s)
		|| !*s;
}
...
RZ_API char *rz_str_version(const char *program) {
	char *s = rz_str_newf ("%s "R2_VERSION" %d @ "
			R_SYS_OS"-"
			R_SYS_ARCH"-%d git.%s\n",
			...
			*R2_GITTAP ? R2_GITTAP: "");
	if (*R2_GITTIP) {
		s = rz_str_appendf (s, "commit: "R2_GITTIP" build: "R2_BIRTH);
	}
	return s;
}
