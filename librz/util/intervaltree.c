...
static int cmp_exact_node(const void *incoming, const RBNode *in_tree, void *user) {
	...
	if (!path_cache->len) {
		...
		while (rz_rbtree_iter_has (path_cache)) {
			RIntervalNode *intervalnode = rz_rbtree_iter_get (path_cache, RIntervalNode, node);
...
RZ_API RBIter rz_interval_tree_first_at(RIntervalTree *tree, ut64 start) {
	...
	RIntervalNode *top_intervalnode = rz_interval_tree_node_at (tree, start);
	if (!top_intervalnode) {
		return it;
	}
...
RZ_API RIntervalNode *rz_interval_tree_node_at_data(RIntervalTree *tree, ut64 start, void *data) {
	...
	while (rz_rbtree_iter_has (&it)) {
		RIntervalNode *intervalnode = rz_rbtree_iter_get (&it, RIntervalNode, node);
...
RZ_API bool rz_interval_node_all_in(RIntervalNode *node, ut64 value, bool end_inclusive, RIntervalIterCb cb, void *user) {
...
static bool rz_interval_node_all_intersect(RIntervalNode *node, ut64 start, ut64 end, bool end_inclusive, RIntervalIterCb cb, void *user) {
