...
static RzThreadLock *get_subprocess_lock(void) {
	RzThreadLock *lock;
	do {
		lock = InterlockedCompareExchangePointer((void **)&subproc_mutex, INVALID_POINTER_VALUE, INVALID_POINTER_VALUE);
	} while (!lock);
	return lock;
}
...
RZ_API bool rz_subprocess_init(void) {
	...
	if (ref == 1) {
		...
		if (!lock) {
			InterlockedExchangePointer((void **)&subproc_mutex, INVALID_POINTER_VALUE);
			InterlockedDecrement(&refcount);
			return false;
		}
		// Enter lock before making it available, so we are the first to run
		rz_th_lock_enter(lock);
		InterlockedExchangePointer((void **)&subproc_mutex, lock);
	} else {
...
RZ_API void rz_subprocess_fini(void) {
	...
	do {
		...
		lock = InterlockedExchangePointer((void **)&subproc_mutex, NULL);
	} while (!lock);
