...
#define SINGLEV(val)       rz_il_op_new_float_from_f32(val)
#define DOUBLEV(val)       rz_il_op_new_float_from_f64(val)
#define FLOATV(format, bv) rz_il_op_new_float(format, bv)
#define FLOATV16(bv)       rz_il_op_new_float(RZ_FLOAT_IEEE754_BIN_16, bv)
#define FLOATV32(bv)       rz_il_op_new_float(RZ_FLOAT_IEEE754_BIN_32, bv)
#define FLOATV64(bv)       rz_il_op_new_float(RZ_FLOAT_IEEE754_BIN_64, bv)
#define FLOATV128(bv)      rz_il_op_new_float(RZ_FLOAT_IEEE754_BIN_128, bv)

#define FBITS(f)                       rz_il_op_new_fbits(f)
#define IS_FINITE(f)                   rz_il_op_new_is_finite(f)
#define IS_NAN(f)                      rz_il_op_new_is_nan(f)
#define IS_INF(f)                      rz_il_op_new_is_inf(f)
#define IS_FZERO(f)                    rz_il_op_new_is_fzero(f)
#define IS_FNEG(f)                     rz_il_op_new_is_fneg(f)
#define IS_FPOS(f)                     rz_il_op_new_is_fpos(f)
#define FNEG(f)                        rz_il_op_new_fneg(f)
#define FABS(f)                        rz_il_op_new_fabs(f)
#define FSUCC(f)                       rz_il_op_new_fsucc(f)
#define FPRED(f)                       rz_il_op_new_fpred(f)
#define FLOAT_TO_UN(len, rmode, f)     rz_il_op_new_fcast_int(len, rmode, f)
#define FLOAT_TO_SN(len, rmode, f)     rz_il_op_new_fcast_sint(len, rmode, f)
#define UN_TO_FLOAT(format, rmode, bv) rz_il_op_new_fcast_float(format, rmode, bv)
#define SN_TO_FLOAT(format, rmode, bv) rz_il_op_new_fcast_sfloat(format, rmode, bv)
#define FCONVERT_TO(format, rmode, f)  rz_il_op_new_fconvert(format, rmode, f)
#define FORDER(x, y)                   rz_il_op_new_forder(f1, f2)
#define FROUND(rmode, f)               rz_il_op_new_fround(rmode, f)
#define FSQRT(rmode, f)                rz_il_op_new_fsqrt(rmode, f)
#define FADD(rmode, x, y)              rz_il_op_new_fadd(rmode, x, y)
#define FSUB(rmode, x, y)              rz_il_op_new_fsub(rmode, x, y)
#define FMUL(rmode, x, y)              rz_il_op_new_fmul(rmode, x, y)
#define FDIV(rmode, x, y)              rz_il_op_new_fdiv(rmode, x, y)
#define FMOD(rmode, x, y)              rz_il_op_new_fmod(rmode, x, y)
#define FMAD(rmode, x, y)              rz_il_op_new_fmad(rmode, x, y)
