...
static inline void rz_itv_free(RInterval *itv) {
	free (itv);
}

static inline ut64 rz_itv_begin(RInterval itv) {
	return itv.addr;
}

static inline ut64 rz_itv_size(RInterval itv) {
	return itv.size;
}

static inline ut64 rz_itv_end(RInterval itv) {
	return itv.addr + itv.size;
}

// Returns true if itv equals itv2
static inline bool rz_itv_eq(RInterval itv, RInterval itv2) {
	return itv.addr == itv2.addr && itv.size == itv2.size;
}

// Returns true if itv contained addr
static inline bool rz_itv_contain(RInterval itv, ut64 addr) {
	const ut64 end = itv.addr + itv.size;
	return itv.addr <= addr && (!end || addr < end);
}

// Returns true if x is a subset of itv
static inline bool rz_itv_include(RInterval itv, RInterval x) {
	const ut64 end = itv.addr + itv.size;
	return itv.addr <= x.addr && (!end || (x.addr + x.size && x.addr + x.size <= end));
}

// Returns true if itv and x overlap (implying they are non-empty)
static inline bool rz_itv_overlap(RInterval itv, RInterval x) {
	const ut64 end = itv.addr + itv.size, end1 = x.addr + x.size;
	return (!end1 || itv.addr < end1) && (!end || x.addr < end);
}

static inline bool rz_itv_overlap2(RInterval itv, ut64 addr, ut64 size) {
