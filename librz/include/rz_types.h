...
#define R_NEWCOPY(x,y) (x*)rz_new_copy(sizeof(x), y)
...
#define rz_ref(x) x->R_REF_NAME++;
#define rz_ref_init(x) x->R_REF_NAME = 1
#define rz_unref(x,f) { assert (x->R_REF_NAME> 0); if (!--(x->R_REF_NAME)) { f(x); } }
