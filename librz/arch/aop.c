...
RZ_API void rz_asm_op_init(RZ_NULLABLE RzAsmOp *op) {
	...
	rz_asm_op_fini(op);
	op->buf = rz_strbuf_new(NULL);
	op->buf_asm = rz_strbuf_new(NULL);
	// buf_inc is set by the user
	op->buf_inc = NULL;
	op->asm_toks = NULL;
}

/**
 * \brief Finishes an RzAsmOp. This effectively frees all members in \p op or sets them to 0.
 *
 * \param op The RzAsmOp to finish.
 */
RZ_API void rz_asm_op_fini(RZ_NULLABLE RzAsmOp *op) {
	...
	op->bitsize = 0;
	op->payload = 0;
	rz_strbuf_free(op->buf);
	op->buf = NULL;
	rz_strbuf_free(op->buf_asm);
	op->buf_asm = NULL;
