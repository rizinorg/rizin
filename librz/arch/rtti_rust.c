...
/**
 * Rust trait object pointers carry a data pointer and a vtable pointer. rustc
 * starts each vtable with drop_in_place (maybe null), size, align.
 * Remaining entries can be methods, vacant slots, or pointers
 * to supertrait vtables used for trait upcasting.
 *
 * This scans data sections for headers and recovers methods
 * from demangled <Type as Trait>::method symbols.
 * Stores the results in classes as "impl Type as Trait"'s for
 * devirtualize_rust.c
 *
 * https://doc.rust-lang.org/reference/types/trait-object.html
 * https://github.com/rust-lang/rust/blob/main/compiler/rustc_middle/src/ty/vtable.rs
 */

#define RUST_VTABLE_HEADER_SLOTS 3

/**
 * \brief Trait method recovered from a vtable slot and its demangled Rust symbol.
 */
...
static char *demangled_name_at(RzAnalysis *analysis, ut64 addr) {
	...
	if (sym && sym->name && analysis->binb.demangle) {
...
static bool parse_demangled_trait_method(const char *demangled, RustTraitMethod *method) {
	...
	method->impl_type = rz_str_ndup(inside, impl_end - inside);
	method->trait_name = rz_str_ndup(trait_start, trait_end - trait_start);
	method->name = rz_str_ndup(suffix, name_end - suffix);
...
static RZ_OWN RzVector /*<RustTraitMethod>*/ *parse_rust_vtable(RVTableContext *context, RzAnalysisVTable *vtable, ut64 end) {
	if (!vtable_header_is_valid(context, vtable->addr)) {
		return NULL;
	}
	...
	ut64 header_size = RUST_VTABLE_HEADER_SLOTS * context->word_size;
	ut64 slot_addr = vtable->addr + header_size;
	vtable->size = header_size;
	for (; end - slot_addr >= context->word_size; slot_addr += context->word_size) {
		...
		bool next_header = end - slot_addr >= header_size && vtable_header_is_valid(context, slot_addr);
		if (!method_addr) {
			if (next_header) {
				break;
			}
		...
		if (!addr_in_text_section(context->analysis, method_addr)) {
			RzBinSection *section = context->analysis->binb.get_vsect_at(context->analysis->binb.bin, method_addr);
			if (!section || !section_can_contain_vtables(section) ||
				method_addr % context->word_size ||
				section->vsize - (method_addr - section->vaddr) < header_size ||
				!vtable_header_is_valid(context, method_addr)) {
				break;
			}
			vtable->size = slot_addr - vtable->addr + context->word_size;
			continue;
		}

		RustTraitMethod method = {
			.addr = method_addr,
			.vtable_offset = slot_addr - vtable->addr,
		};
		bool parsed = parse_method_name(context->analysis, method_addr, &method);
		if (!parsed && next_header) {
			break;
		}
		vtable->size = slot_addr - vtable->addr + context->word_size;
		if (!parsed) {
...
static void apply_rust_vtable(RVTableContext *context, RzAnalysisVTable *vtable, RzVector /*<RustTraitMethod>*/ *methods) {
...
/**
 * Rust trait object pointers carry a data pointer and a vtable pointer.
 * starts each vtable with drop_in_place (maybe null), size, align.
 * Remaining entries can be methods, vacant slots, or pointers
 * to supertrait vtables used for trait upcasting.
 *
...
RZ_API void rz_analysis_rtti_rust(RZ_NONNULL RzAnalysis *analysis) {
	...
	rz_pvector_foreach (sections, iter) {
		...
		if (section->vsize < RUST_VTABLE_HEADER_SLOTS * context.word_size ||
			section->vsize > UT64_MAX - section->vaddr) {
			continue;
		}

		ut64 addr = RZ_ROUND(section->vaddr, context.word_size);
		ut64 end = section->vaddr + section->vsize;
		while (end - addr >= RUST_VTABLE_HEADER_SLOTS * context.word_size) {
			if (rz_cons_is_breaked()) {
				break;
			}
			RzAnalysisVTable vtable = { .addr = addr };
			RzVector *methods = parse_rust_vtable(&context, &vtable, end);
			if (methods) {
				apply_rust_vtable(&context, &vtable, methods);
				rz_vector_free(methods);
			}
