		...
		case 2:
			if (len > 1) {
				if (arg1 == A_OFFSET) {
					...
					// op arg, @Ri; op arg, Rn
					disasm = rz_str_newf(name, buf[1], buf[0] & mask);
					val1 = buf[1];
				} else if (arg1 == A_BIT) {
					// bit addressing mode
					disasm = rz_str_newf(name, arg_bit(buf[1]), buf[1] & 0x07);
					val1 = buf[1];
				} else {
					// direct, immediate, bit
					disasm = rz_str_newf(name, buf[1]);
					val1 = buf[1];
				}
			} else {
				*olen = -1;
		...
		case 3:
			if (len > 2) {
				if (arg1 == A_ADDR16) {
					...
					disasm = rz_str_newf(name, buf[2], buf[1]);
					val1 = buf[2];
					val2 = buf[1];
				} else {
					// direct, immediate
					disasm = rz_str_newf(name, buf[1], buf[2]);
					val1 = buf[1];
				}
			} else {
				*olen = -1;
