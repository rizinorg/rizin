...
//     Are trivially always valid.  The only execution flow that could invalidate a reservation
//     between lr and sc are external ones — another hart, a DMA engine, an interrupt
//     handler that writes to the same address, memory-mapped I/O side-effects, etc. —
//     none of which exist in this model.  lr.* is therefore a plain load and sc.*
//     is an unconditional store that always reports success (rd = 0).
//
...
//     SEQ-based implementation is fully correct and complete.
//
