...
typedef enum {
	...
	C6X_OP_IMM, ///< immediate constant (signed value in the imm member)
	C6X_OP_CTRLREG, ///< control register named by the ctrl member (MVC)
...
/** One decoded operand; the kind selects which union member is valid. */
typedef struct {
	C6xOpKind kind;
	union {
