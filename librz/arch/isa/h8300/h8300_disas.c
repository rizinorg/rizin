...
static int decode_r32_4(const ut8 *bytes, struct h8300_cmd *cmd) {
	int ret = 4;
	cmd->fmt = H8300_INSN_FORMAT_R32;
...
static int decode_i3ri32(const ut8 *bytes, struct h8300_cmd *cmd) {
	...
	OPS_ADD(H8300_OP_RI, reg, bytes[1] >> 4);
	return ret;
}
...
static int h8300_decode_4(const ut8 *instr, struct h8300_cmd *cmd) {
	...
	switch (x4 & 0xffffff8f) {
		CASE_F_F_CCR(decode_r32_4, 0x01406900, LDC_W);
...
static int h8300_decode_2(const ut8 *instr, struct h8300_cmd *cmd) {
	...
	switch (x2 & 0xfff8) {
		CASE_F_F(decode_r16_2, 0x6d70, POP);
		CASE_F_F(decode_r16_2, 0x6df0, PUSH);
