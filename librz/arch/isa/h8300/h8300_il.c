...
static RzILOpEffect *op_mov_b(H8300Cmd *cmd) {
	...
	case H8300_INSN_FORMAT_ABSR8:
		return SEQ2(R8_X(1, LOAD(U16_OP(0))), ccr_unary_NZV0(8, LOAD(U16_OP(0))));
	case H8300_INSN_FORMAT_R8ABS:
		return SEQ2(STORE(U16_OP(1), R8_OP(0)), ccr_unary_NZV0(8, R8_OP(0)));
	case H8300_INSN_FORMAT_IMMR8:
		return SEQ2(R8_X(1, U8_OP(0)), ccr_unary_NZV0(8, U8_OP(0)));
...
static RzILOpEffect *op_mov_w(H8300Cmd *cmd) {
	...
	case H8300_INSN_FORMAT_IMMR16:
		return SEQ2(R16_X(1, U16_OP(0)), ccr_unary_NZV0(16, U16_OP(0)));
	...
	case H8300_INSN_FORMAT_R16ABS:
		return SEQ2(STOREW(U16_OP(1), R16_OP(0)), ccr_unary_NZV0(16, R16_OP(0)));
...
static RzILOpEffect *aop(RzAnalysis *a, RzAnalysisOp *op, H8300Cmd *cmd) {
	...
	case H8300_INSN_SHAL: return SEQ3(
		SETL("result", SHIFTL0(R8_OP(0), U8(1))),
		ccr_unary_NZVC(8, VARL("result"),
			XOR(MSB(VARL("result")), MSB(R8_OP(0))),
			MSB(R8_OP(0))),
		R8_X(0, VARL("result")));
	case H8300_INSN_SHAR: return SEQ3(
		SETL("result", SHIFTRA(R8_OP(0), U8(1))),
		ccr_unary_NZVC(8, VARL("result"),
			IL_FALSE,
			LSB(R8_OP(0))),
		R8_X(0, VARL("result")));
	case H8300_INSN_SHLL: return SEQ3(
		SETL("result", SHIFTL0(R8_OP(0), U8(1))),
		ccr_unary_NZVC(8, VARL("result"),
			IL_FALSE,
			MSB(R8_OP(0))),
		R8_X(0, VARL("result")));
	case H8300_INSN_SHLR: return SEQ3(
