...
static inline bool m68k_mode_has_master_stack(cs_mode mode) {
	/* MC68060UM 11.1.2: the 68060 retains SR.M for software use, but has
	 * only one supervisor stack. */
...
static bool is_supervisor_stack_bank(const M68KILCtx *ctx, m68k_reg reg) {
	rz_return_val_if_fail(ctx, false);
	return m68k_mode_has_master_stack(ctx->mode) &&
		(reg == M68K_REG_ISP || reg == M68K_REG_MSP);
}

static RzILOpBool *supervisor_stack_bank_active(m68k_reg reg) {
	/* M68000PRM 1.3.1/1.3.2: ISP and MSP name the physical A7 banks.
	 * Their globals hold saved values only while the bank is inactive. */
	return AND(sr_bit(M68K_SR_S), reg == M68K_REG_MSP ? sr_bit(M68K_SR_M) : INV(sr_bit(M68K_SR_M)));
}
...
RZ_IPI RzILOpPure *m68k_reg_value(M68KILCtx *ctx, m68k_reg reg) {
	...
	if (is_supervisor_stack_bank(ctx, reg)) {
		return ITE(supervisor_stack_bank_active(reg), VARG("a7"), VARG(name));
	}
	return VARG(name);
}
...
RZ_IPI RzILOpPure *m68k_read_reg_sized(M68KILCtx *ctx, m68k_reg reg, ut32 bits) {
	...
	case M68K_REG_MSP:
	case M68K_REG_ISP:
		return bits == 32 ? m68k_reg_value(ctx, reg) : UNSIGNED(bits, m68k_reg_value(ctx, reg));
	case M68K_REG_CCR:
		return bits <= 8
			? UNSIGNED(bits, VARG("sr"))
			: UNSIGNED(bits, UNSIGNED(8, VARG("sr")));
	case M68K_REG_SR:
...
static RzILOpBool *coldfire_supervisor_stack(RzILOpPure *sr) {
	/* MCF5208RM 3.2.3 and 5.2.1: the second A7 bank participates only
	 * while CACR[EUSP] is enabled. */
	return AND(
		NON_ZERO(LOGAND(UNSIGNED(16, sr), U16(1u << M68K_SR_S))),
		cacr_eusp(VARG("cacr")));
}
...
RZ_IPI RzILOpEffect *m68k_write_reg_sized(M68KILCtx *ctx, m68k_reg reg, ut32 bits, RzILOpPure *value) {
	...
	if (is_supervisor_stack_bank(ctx, reg)) {
		RzILOpPure *sp = UNSIGNED(32, value);
		return BRANCH(supervisor_stack_bank_active(reg), SETG("a7", DUP(sp)), SETG(name, sp));
	}
	if (reg == M68K_REG_CACR && rz_m68k_mode_is_coldfire(ctx->mode)) {
		/* Changing EUSP in supervisor mode changes which physical A7 is
		 * active, so preserve the outgoing bank before loading the incoming
		 * one. */
		RzILOpEffect *enable_eusp = SEQ2(
			SETG("usp", UNSIGNED(32, VARG("a7"))),
			SETG("a7", UNSIGNED(32, VARG("isp"))));
		RzILOpEffect *disable_eusp = SEQ2(
			SETG("isp", UNSIGNED(32, VARG("a7"))),
			SETG("a7", UNSIGNED(32, VARG("usp"))));
		RzILOpEffect *switch_sp = BRANCH(
			cacr_eusp(VARG("cacr")),
			disable_eusp,
			enable_eusp);
		return SEQ3(
			SETL("cacr_write_value", UNSIGNED(32, value)),
			BRANCH(
				AND(
					m68k_supervisor_mode(),
					XOR(cacr_eusp(VARG("cacr")), cacr_eusp(VARL("cacr_write_value")))),
				switch_sp,
				EMPTY()),
			SETG("cacr", VARL("cacr_write_value")));
	}
	if (reg == M68K_REG_SR) {
		if (rz_m68k_mode_is_coldfire(ctx->mode)) {
			RzILOpEffect *save_old_sp = BRANCH(
				coldfire_supervisor_stack(VARG("sr")),
				SETG("isp", UNSIGNED(32, VARG("a7"))),
				SETG("usp", UNSIGNED(32, VARG("a7"))));
			RzILOpEffect *load_new_sp = BRANCH(
				coldfire_supervisor_stack(VARL("sr_write_value")),
				SETG("a7", UNSIGNED(32, VARG("isp"))),
				SETG("a7", UNSIGNED(32, VARG("usp"))));
			RzILOpPure *old_bank = ITE(coldfire_supervisor_stack(VARG("sr")), U8(1), U8(0));
			RzILOpPure *new_bank = ITE(coldfire_supervisor_stack(VARL("sr_write_value")), U8(1), U8(0));
			return SEQ3(
...
static ut32 hidden_extension_bytes_before_ea(const M68KILCtx *ctx) {
	...
	case M68K_INS_DIVSL:
	case M68K_INS_DIVUL:
		/* M68000PRM long MUL/DIV: the register/size extension precedes EA. */
		return rz_m68k_detail_op_bits(ctx->m68k, 16) == 32 ? 2 : 0;
	case M68K_INS_BFCHG:
	case M68K_INS_BFCLR:
...
RZ_IPI M68KEA m68k_effective_addr(M68KILCtx *ctx, const cs_m68k_op *op, ut32 bits) {
	...
	case M68K_AM_REGI_ADDR_POST_INC:
		ea.addr = m68k_reg_value(ctx, base_reg);
		ea.post = m68k_set_addr_reg_delta(ctx, base_reg, rz_m68k_reg_stack_access_bytes(ctx->mode, base_reg, bits));
		break;
	case M68K_AM_REGI_ADDR_PRE_DEC:
		ea.pre = m68k_set_addr_reg_delta(ctx, base_reg, -(st32)rz_m68k_reg_stack_access_bytes(ctx->mode, base_reg, bits));
		ea.addr = m68k_reg_value(ctx, base_reg);
		break;
	case M68K_AM_REGI_ADDR_DISP:
	...
	case M68K_AM_PCI_INDEX_BASE_DISP: {
		...
		idx = index_value(ctx, op);
		ut64 pc_base = op->address_mode == M68K_AM_PCI_INDEX_BASE_DISP && base_reg == M68K_REG_INVALID
			? 0
			: pc_relative_base(ctx, op);
		ea.addr = ADD(U32(pc_base + disp), idx);
		break;
	}
	case M68K_AM_PC_MEMI_PRE_INDEX:
		base = base_reg != M68K_REG_INVALID ? U32(pc_relative_base(ctx, op)) : U32(0);
		idx = index_value(ctx, op);
		ea.addr = ADD(LOADW(32, ADD(ADD(base, idx), S32((st32)op->mem.in_disp))), S32((st32)op->mem.out_disp));
		break;
	case M68K_AM_PC_MEMI_POST_INDEX:
