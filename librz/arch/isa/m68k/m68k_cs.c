...
RZ_IPI ut32 rz_m68k_reg_stack_access_bytes(cs_mode mode, m68k_reg reg, ut32 bits) {
	...
	// stack word alignment on classic M680x0. ColdFire treats A7 like every
	// other address register and adjusts it by one for byte operands (CFPRM).
	return reg == M68K_REG_A7 && bytes == 1 && !rz_m68k_mode_is_coldfire(mode) ? 2 : bytes;
}
