...
static bool __typeLoad(void *p, const char *k, const char *v) {
	...
	if (!strncmp(v, "struct", 6) && strncmp(k, "struct.", 7)) {
		...
		char *members = sdb_get(t->sdb_types, query, 0);
		...
		if (members) {
			do {
				...
				char *subtype = sdb_get(t->sdb_types, query, 0);
				if (!subtype) {
					break;
				}
				...
				if (tmp) {
					*tmp++ = 0;
					...
					btype = sdb_num_get(t->sdb_types, query, 0);
...
RZ_API char *rz_type_parse_c_file(RzType *t, const char *path, const char *dir, char **error_msg) {
	char *str = NULL;
	TCCState *T = tcc_new(t->target->cpu, t->target->bits, t->target->os);
	if (!T) {
		return NULL;
	}
	tcc_set_callback(T, &__appendString, &str);
	tcc_set_error_func(T, (void *)error_msg, __errorFunc);
	sdb_foreach(t->sdb_types, __typeLoad, t);
...
RZ_API char *rz_type_parse_c_string(RzType *t, const char *code, char **error_msg) {
	char *str = NULL;
	TCCState *T = tcc_new(t->target->cpu, t->target->bits, t->target->os);
	if (!T) {
		return NULL;
	...
	tcc_set_callback(T, &__appendString, &str);
	tcc_set_error_func(T, (void *)error_msg, __errorFunc);
	sdb_foreach(t->sdb_types, __typeLoad, NULL);
