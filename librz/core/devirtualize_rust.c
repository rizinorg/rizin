...
/**
 * \brief Recovered Any trait vtable and concrete type information for downcast annotations.
 */
...
/**
 * \brief Recovered caller argument values used to replay a trait object call.
 */
...
/**
 * \brief Recovered trait vtable and its validated methods used for devirtualization.
 */
...
/**
 * \brief Recovered method and vtable slot used to resolve a virtual call.
 */
	...
	rz_pvector_foreach (classes, iter) {
		...
		rz_vector_foreach (class_vtables, analysis_vtable) {
			...
			rz_vector_foreach (class_methods, analysis_method) {
				if (analysis_method->vtable_offset < 0) {
					continue;
				}
				ut64 offset = analysis_method->vtable_offset;
				ut8 slot[8];
				if (offset % word_size || offset > UT64_MAX - vtable.addr ||
					rz_io_nread_at(core->io, vtable.addr + offset, slot, word_size) != word_size ||
					rz_read_ble(slot, rz_analysis_is_big_endian_set(core->analysis), bits) != analysis_method->addr) {
					continue;
				}
				if (is_any_trait_name(trait_name) && method_name_is_type_id(analysis_method->name, analysis_method->real_name)) {
					push_any_vtable_from_class(core, any_vtables, class_name, analysis_vtable, analysis_method);
				}
...
static bool rust_virtual_comment_emit(void *user, const ut64 addr, const void *value) {
	...
	ht_sp_foreach(set, rust_virtual_comment_add_name, &ctx);
...
static void devirtualize_rust_function(RzCore *core, RzAnalysisFunction *function, RustVTableIndex *vtable_index, RzVector /*<RustAnyVTable>*/ *any_vtables, RZ_NULLABLE const RustCallSeed *seed, HtUP *virtual_calls) {
	...
	while (start < end) {
		...
		} else if (!track_step_or_skip(core, op, next)) {
			break;
		}
...
static bool caller_replay_step(RzCore *core, RzAnalysisOp *op, ut64 next_addr, const char *ret_reg) {
	...
	return track_step_or_skip(core, op, next_addr);
}
...
static void collect_caller_args_from_site(RzCore *core, RzAnalysisFunction *caller, ut64 call_addr, RustVTableIndex *vtable_index, const RustCallSeed *args, RzVector /*<RustCallSeed>*/ *seeds) {
	...
	while (start < end) {
		...
		ut64 next = start + op->size;
		if (!caller_replay_step(core, op, next, ret_reg)) {
			replay_ok = false;
			break;
		}
		start = next;
