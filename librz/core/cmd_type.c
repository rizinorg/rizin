...
static RzCmdStatus types_enum_member_find(RzCore *core, const char *enum_name, const char *enum_value) {
	...
	char *enum_member = rz_type_enum_member(core->analysis->type, enum_name, NULL, value);
	if (!enum_member) {
...
static RzCmdStatus types_enum_member_find_all(RzCore *core, const char *enum_value) {
	...
	ut64 value = rz_num_math(core->num, enum_value);
	RzList *matches = rz_type_enum_find_member(core->analysis->type, value);
...
static bool print_type_c(RzCore *core, const char *ctype) {
	...
	if (name && type) {
		...
		if (rz_str_startswith(type, "struct")) {
			rz_types_struct_print_c(core->analysis->type, name, true);
		} else if (rz_str_startswith(type, "union")) {
			rz_types_union_print_c(core->analysis->type, name, true);
		} else if (rz_str_startswith(type, "enum")) {
			rz_types_enum_print_c(core->analysis->type, name, true);
		} else if (rz_str_startswith(type, "typedef")) {
			rz_types_typedef_print_c(core->analysis->type, name);
		} else if (rz_str_startswith(type, "func")) {
			rz_types_function_print(core->analysis->type, name, RZ_OUTPUT_MODE_STANDARD, NULL);
		}
		return true;
	}
	return false;
...
RZ_IPI int rz_cmd_type(void *data, const char *input) {
	...
	case 'u': { // "tu"
		...
		case 'j': // "tuj"
			if (input[2]) {
				...
				rz_types_union_print_json(T);
			}
			break;
		case 'c': // "tuc"
			rz_types_union_print_c(T, rz_str_trim_head_ro(input + 2), true);
			break;
		case 'd': // "tud"
			rz_types_union_print_c(T, rz_str_trim_head_ro(input + 2), false);
			break;
		case ' ': // "tu "
			rz_core_types_show_format(core, rz_str_trim_head_ro(input + 1), RZ_OUTPUT_MODE_STANDARD);
			break;
		case 0:
			rz_types_union_print_sdb(T);
			break;
		}
	} break;
	case 'k': // "tk"
		res = (input[1] == ' ')
			? rz_type_kuery(T, input + 2)
			: rz_type_kuery(T, "*");
		if (res) {
			rz_cons_print(res);
			free(res);
	...
	case 's': { // "ts"
		...
		case 's':
			if (input[2] == ' ') {
				rz_cons_printf("%" PFMT64u "\n", (rz_type_get_bitsize(T, input + 3) / 8));
			} else {
				rz_core_cmd_help(core, help_msg_ts);
			}
			break;
		case 0:
			rz_types_struct_print_sdb(T);
			break;
		case 'c': // "tsc"
			rz_types_struct_print_c(T, rz_str_trim_head_ro(input + 2), true);
			break;
		case 'd': // "tsd"
			rz_types_struct_print_c(T, rz_str_trim_head_ro(input + 2), false);
			break;
		case 'j': // "tsj"
	...
	case 'e': { // "te"
		...
		if (name && (rz_type_kind(T, name) != RZ_BASE_TYPE_KIND_ENUM)) {
		...
		case 'k': // "tek"
			...
			break;
		case 'b': // "teb"
			res = rz_type_enum_member(T, name, member_value, 0);
			break;
		case 'c': // "tec"
			rz_types_enum_print_c(T, rz_str_trim_head_ro(input + 2), true);
			break;
		case 'd': // "ted"
			rz_types_enum_print_c(T, rz_str_trim_head_ro(input + 2), false);
			break;
		case 'f': // "tef"
			if (member_value) {
	...
	case 'o': // "to"
		...
		if (input[1] == ' ') {
			...
			rz_type_load_sdb(core->analysis->type, input + 3);
	...
	case 'l': // "tl"
		...
		case '-':
			switch (input[2]) {
			case '*':
				rz_type_unlink_all(T);
				break;
			case ' ': {
				const char *ptr = input + 3;
				ut64 addr = rz_num_math(core->num, ptr);
				rz_type_unlink(T, addr);
				break;
			}
			}
			break;
	...
	case '-': // "t-"
		if (input[1] == '?') {
			...
			rz_type_purge(core->analysis->type);
			rz_type_parse_c_reset(core->analysis->type);
		} else {
			const char *name = rz_str_trim_head_ro(input + 1);
			if (*name) {
				rz_type_remove_parsed_type(core->analysis->type, name);
			} else {
	...
	case 'f': // "tf"
		...
		case 'k': // "tfk"
			if (input[2] == ' ') {
				...
				rz_types_function_print(T, name, RZ_OUTPUT_MODE_SDB, NULL);
			} else {
		...
		case 'j': // "tfj"
			if (input[2] == ' ') {
				...
				PJ *pj = rz_core_pj_new(core);
				rz_types_function_print(T, name, RZ_OUTPUT_MODE_JSON, pj);
				pj_end(pj);
				rz_cons_println(pj_string(pj));
				pj_free(pj);
			} else {
	...
	case 't': { // "tt"
		...
		if (input[1] == 'c') { // "ttc"
			rz_types_typedef_print_c(T, input + 2);
			break;
		}
		if (input[1] == '?') { // "tt?"
...
RZ_IPI RzCmdStatus rz_type_del_handler(RzCore *core, int argc, const char **argv) {
	rz_type_remove_parsed_type(core->analysis->type, argv[1]);
	return RZ_CMD_STATUS_OK;
}
...
RZ_IPI RzCmdStatus rz_type_enum_bitfield_handler(RzCore *core, int argc, const char **argv) {
	...
	char *output = rz_type_enum_member(core->analysis->type, enum_name, enum_member, 0);
	if (!output) {
...
RZ_IPI RzCmdStatus rz_type_enum_c_handler(RzCore *core, int argc, const char **argv) {
	const char *enum_name = argc > 1 ? argv[1] : NULL;
	rz_types_enum_print_c(core->analysis->type, enum_name, true);
	return RZ_CMD_STATUS_OK;
}

RZ_IPI RzCmdStatus rz_type_enum_c_nl_handler(RzCore *core, int argc, const char **argv) {
	const char *enum_name = argc > 1 ? argv[1] : NULL;
	rz_types_enum_print_c(core->analysis->type, enum_name, false);
	return RZ_CMD_STATUS_OK;
}
...
RZ_IPI RzCmdStatus rz_type_list_function_handler(RzCore *core, int argc, const char **argv, RzOutputMode mode) {
	...
	if (function) {
		PJ *pj = (mode == RZ_OUTPUT_MODE_JSON) ? rz_core_pj_new(core) : NULL;
		rz_types_function_print(core->analysis->type, function, mode, pj);
		if (mode == RZ_OUTPUT_MODE_JSON) {
			rz_cons_println(pj_string(pj));
			pj_free(pj);
		}
...
RZ_IPI RzCmdStatus rz_type_kuery_handler(RzCore *core, int argc, const char **argv) {
	...
	char *output = rz_type_kuery(core->analysis->type, query);
	if (!output) {
...
RZ_IPI RzCmdStatus rz_type_open_sdb_handler(RzCore *core, int argc, const char **argv) {
	rz_type_load_sdb(core->analysis->type, argv[1]);
	return RZ_CMD_STATUS_OK;
}
...
RZ_IPI RzCmdStatus rz_type_list_structure_handler(RzCore *core, int argc, const char **argv, RzOutputMode mode) {
	...
	if (typename) {
		...
		if (mode == RZ_OUTPUT_MODE_RIZIN) {
			...
			rz_types_struct_print_json(core->analysis->type);
		} else {
			rz_types_struct_print_sdb(core->analysis->type);
		}
	}
	return RZ_CMD_STATUS_OK;
}
...
RZ_IPI RzCmdStatus rz_type_structure_c_handler(RzCore *core, int argc, const char **argv) {
	const char *typename = argc > 1 ? argv[1] : NULL;
	rz_types_struct_print_c(core->analysis->type, typename, true);
	return RZ_CMD_STATUS_OK;
}

RZ_IPI RzCmdStatus rz_type_structure_c_nl_handler(RzCore *core, int argc, const char **argv) {
	const char *typename = argc > 1 ? argv[1] : NULL;
	rz_types_struct_print_c(core->analysis->type, typename, false);
	return RZ_CMD_STATUS_OK;
}
...
RZ_IPI RzCmdStatus rz_type_typedef_c_handler(RzCore *core, int argc, const char **argv) {
	...
	RzType *T = core->analysis->type;
	if (!typename) {
		rz_types_typedef_print_c(T, NULL);
		return RZ_CMD_STATUS_OK;
	}
	rz_types_typedef_print_c(T, typename);
	return RZ_CMD_STATUS_OK;
}
...
RZ_IPI RzCmdStatus rz_type_union_c_handler(RzCore *core, int argc, const char **argv) {
	const char *typename = argc > 1 ? argv[1] : NULL;
	rz_types_union_print_c(core->analysis->type, typename, true);
	return RZ_CMD_STATUS_OK;
}

RZ_IPI RzCmdStatus rz_type_union_c_nl_handler(RzCore *core, int argc, const char **argv) {
	const char *typename = argc > 1 ? argv[1] : NULL;
	rz_types_union_print_c(core->analysis->type, typename, false);
	return RZ_CMD_STATUS_OK;
}
