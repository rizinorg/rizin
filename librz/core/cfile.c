...
static bool core_file_do_load_for_debug(RzCore *r, ut64 loadaddr,
                                        const char *filenameuri);
static bool core_file_do_load_for_io_plugin(RzCore *r, ut64 baseaddr,
                                            ut64 loadaddr);
...
#define __is_inside_section(item_addr, section)                                \
  (item_addr >= old_baddr_shift + section->vaddr &&                            \
   item_addr <= old_baddr_shift + section->vaddr + section->vsize)
...
static bool __rebase_flags(RzFlagItem *flag, void *user) {
  ...
  RzListIter *it;
  RzBinSection *sec;
  // Only rebase flags that were in the rebased sections, otherwise it will take
  // too long
  rz_list_foreach(reb->sections_backup, it, sec) {
    if (__is_inside_section(flag->offset, sec)) {
      rz_flag_set(reb->core->flags, flag->name, flag->offset + reb->diff,
                  flag->size);
      break;
    }
  }
 ...
 */
RZ_API void rz_core_rebase_everything(
    RzCore *core, RZ_NONNULL RZ_BORROW RzList *sections_backup,
    bool infer_new_baddr_shift, ut64 old_baddr_shift, ut64 new_baddr_shift) {
  ...
  rz_list_foreach(core->analysis->fcns, it, fcn) {
    rz_list_foreach(sections_backup, itit, old_section) {
      ...
      rz_list_foreach(fcn->bbs, ititit, bb) {
        if (bb->addr >= new_sec_addr &&
            bb->addr <= new_sec_addr + old_section->vsize) {
...
RZ_API bool rz_core_file_open_load(RZ_NONNULL RzCore *core,
                                   RZ_NONNULL const char *filepath, ut64 addr,
                                   int perms, bool write_mode) {
...
RZ_API void rz_core_file_reopen_debug(RzCore *core, const char *args) {
  ...
  // Reopen the original file as read only since we can't open native debug
  // while the file is open with write permissions
...
RZ_API bool rz_core_file_reopen(RzCore *core, const char *args, int perm,
  ...
  RzCoreFile *ofile = core->file;
  RzBinFile *bf = ofile ? rz_bin_file_find_by_fd(core->bin, ofile->fd) : NULL;
  RzIODesc *odesc =
      (core->io && ofile) ? rz_io_desc_get(core->io, ofile->fd) : NULL;
  char *ofilepath = NULL,
       *obinfilepath = (bf && bf->file) ? strdup(bf->file) : NULL;
  int ret = false;
  ...
  if (file) {
    ...
    if (core->bin->cur && core->io && rz_io_desc_get(core->io, file->fd) &&
        !loadbin) {
      // force here NULL because is causing uaf look this better in future XXX
      // @alvarofe
      core->bin->cur = NULL;
    }
    // close old file
...
RZ_API void rz_core_sysenv_begin(RzCore *core) {
  ...
  rz_sys_setenv("RZ_COLOR",
                rz_config_get_i(core->config, "scr.color") ? "1" : "0");
  rz_sys_setenv("RZ_DEBUG",
                rz_config_get_b(core->config, "cfg.debug") ? "1" : "0");
...
static bool core_file_do_load_for_debug(RzCore *r, ut64 baseaddr,
                                        const char *filenameuri) {
  ...
  opt.obj_opts.elf_load_sections =
      rz_config_get_b(r->config, "elf.load.sections");
  opt.obj_opts.elf_checks_sections =
      rz_config_get_b(r->config, "elf.checks.sections");
  opt.obj_opts.elf_checks_segments =
      rz_config_get_b(r->config, "elf.checks.segments");
...
static bool core_file_do_load_for_io_plugin(RzCore *r, ut64 baseaddr,
  ...
  if (rz_core_bin_apply_all_info(r, binfile)) {
    if (!r->analysis->sdb_cc->path) {
      RZ_LOG_WARN("No calling convention defined for this file, analysis may "
                  "be inaccurate.\n");
    }
  }
  plugin = rz_bin_file_cur_plugin(binfile);
...
RZ_API bool rz_core_file_loadlib(RzCore *core, const char *lib, ut64 libaddr) {
  ...
  const char *ldlibrarypath[] = {dirlibs,          libdir,
#ifndef __WINDOWS__
...
static bool map_multi_dex(RzCore *core, RzIODesc *desc, ut32 id) {
  ...
  RzBinFile *cur =
      rz_pvector_empty(&cf->binfiles) ? NULL : rz_pvector_tail(&cf->binfiles);
  if (cur) {
...
RZ_API bool rz_core_bin_load(RZ_NONNULL RzCore *r,
                             RZ_NULLABLE const char *filenameuri, ut64 baddr) {
  ...
  if (is_io_load) {
    ...
    if ((desc->plugin && desc->plugin->isdbg) ||
        rz_config_get_b(r->config, "cfg.debug")) {
      ...
      if (!strncmp(filenameuri, "apk://", 6) && r->io->files->size > 1) {
        ...
        rz_id_storage_foreach(r->io->files, (RzIDStorageForeachCb)map_multi_dex,
                              r);
  ...
  if (plugin && plugin->name) {
    if (!strcmp(plugin->name, "any")) {
      if (rz_str_startswith(desc->name, "rap") && strstr(desc->name, "://")) {
        ...
        rz_io_map_new(r->io, desc->fd, desc->perm, 0, laddr,
  ...
  if (rz_config_get_b(r->config, "bin.libs")) {
    ...
    rz_list_foreach(imports, iter, imp) {
      ...
      rz_id_storage_foreach(r->io->files,
                            (RzIDStorageForeachCb)resolve_import_cb, &linkdata);
 ...
 */
RZ_API RZ_BORROW RzCoreFile *
rz_core_file_open_many(RZ_NONNULL RzCore *r, RZ_NULLABLE const char *file,
                       int perm, ut64 base_address) {
...
/**
 * \brief Tries to open the file as is, otherwise tries as is a compilation of
 * files
 *
 * Calls rz_io_open_nomap but if it fails, then tries with
 * rz_core_file_open_many; Also, loadaddr is rizin -m (mapaddr)
 */
RZ_API RZ_BORROW RzCoreFile *rz_core_file_open(RZ_NONNULL RzCore *r,
                                               RZ_NONNULL const char *file,
                                               int flags, ut64 loadaddr) {
...
RZ_API void rz_core_file_close(RzCoreFile *fh) {
  ...
  while (!rz_pvector_empty(&fh->extra_files)) {
    ...
    rz_io_desc_close(
        rz_pvector_at(&fh->extra_files, rz_pvector_len(&fh->extra_files) - 1));
  }
  while (!rz_pvector_empty(&fh->binfiles)) {
    // same as for maps above
    rz_bin_file_delete(
        r->bin,
        rz_pvector_at(&fh->binfiles, rz_pvector_len(&fh->binfiles) - 1));
  }
  if (r->file == fh) {
    r->file = NULL;
  }
...
RZ_API bool rz_core_file_print(RzCore *core, RzOutputMode mode) {
  ...
  rz_list_foreach(core->files, iter, f) {
    ...
    default: {
      ...
      rz_cons_printf(fmt, core->io->desc->fd == f->fd ? '*' : '-', count,
...
RZ_API int rz_core_file_binlist(RzCore *core) {
  ...
  rz_list_foreach(binfiles, iter, binfile) {
    ...
    if (cf) {
      rz_cons_printf("%c %d %s ; %s\n", core->io->desc == desc ? '*' : '-', fd,
                     desc->uri, desc->perm & RZ_PERM_W ? "rw" : "r");
    }
  }
...
RZ_IPI void rz_core_io_map_info_free(RzCoreIOMapInfo *info) { free(info); }
