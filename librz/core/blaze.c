...
static int __isdata(RzCore *core, ut64 addr) {
	if (!rz_io_is_valid_offset (core->io, addr, false)) {
	...
	RPVector *list = rz_meta_get_all_in (core->anal, addr, R_META_TYPE_ANY);
	void **it;
	int result = 0;
...
static bool addBB(RzList *block_list, ut64 start, ut64 end, ut64 jump, ut64 fail, bb_type_t type, int score) {
	...
	if (jump < UT64_MAX) {
		...
		rz_list_append (block_list, jump_bb);
	}
	if (fail < UT64_MAX) {
		...
		rz_list_append (block_list, fail_bb);
	}
	rz_list_append (block_list, bb);
	return true;
}
...
void dump_blocks (RzList* list) {
	RzListIter *iter;
	bb_t *block = NULL;
	rz_list_foreach (list, iter, block) {
		dump_block(block);
	}
}
...
static void printFunctionCommands(RzCore *core, fcn_t* fcn, const char *name) {
	if (!fcn) {
	...
	char *_name = name? (char *) name: rz_str_newf ("%s.%" PFMT64x, pfx, fcn->addr);
	...
	rz_list_foreach (fcn->bbs, fcn_iter, cur) {
		rz_cons_printf ("afb+ 0x%08" PFMT64x " 0x%08" PFMT64x " %llu 0x%08"PFMT64x" 0x%08"PFMT64x"\n"
			, fcn->addr, cur->start, cur->end - cur->start, cur->jump, cur->fail);
	}
}

static void createFunction(RzCore *core, fcn_t* fcn, const char *name) {
	if (!fcn) {
...
RZ_API bool core_anal_bbs(RzCore *core, const char* input) {
	...
	while (cur >= base && cur < size) {
		...
		RzAnalOp *const op = rz_core_anal_op (core, dst, R_ANAL_OP_MASK_BASIC | R_ANAL_OP_MASK_DISASM);
	...
	rz_list_sort (block_list, (RzListComparator)bbCMP);
	...
	while (block_list->length > 0) {
		...
		if (block_list->length > 0) {
			bb_t *next_block = (bb_t*) rz_list_iter_get_data (block_list->tail);
			if (!next_block) {
	...
	rz_list_foreach (result, iter, block) {
		if (rz_cons_is_breaked ()) {
			break;
		}
		if (block && (block->reached == 0 || block->called >= 1)) {
			...
			while (!rz_stack_is_empty (stack)) {
				...
				if (cur->jump < UT64_MAX && !sdb_num_get (sdb, Fhandled(cur->jump), NULL)) {
					...
					if (!rz_stack_push (stack, (void*)jump)) {
				...
				if (cur->fail < UT64_MAX && !sdb_num_get (sdb, Fhandled(cur->fail), NULL)) {
					...
					if (!rz_stack_push (stack, (void*)fail)) {
						eprintf ("Failed to push jump block to stack\n");
					}
				}
			}
	...
	for (x = 0; x < 1024; x++) {
		if (lista[x] != 0) {
			...
			while (cur < size) {
				...
				rz_list_foreach (block_list, iter, block) {
