...
static RzList *__xrefs(RzCore *core, ut64 addr) {
	...
	rz_list_foreach (xrefs, iter, ref) {
		...
		RzAnalysisFunction *rf = rz_analysis_get_fcn_in (core->anal, ref->addr, 0);
		item->fcn = rf;
		if (rf) {
			item->name = rf->name;
		}
...
static void __reset_status(RzCoreVisualViewGraph *status) {
	...
	status->fcn = rz_analysis_get_function_at (status->core->anal, status->addr);
...
RZ_API int rz_core_visual_view_graph(RzCore *core) {
	...
	RzAnalysisFunction *fcn = rz_analysis_get_fcn_in (core->anal, status.addr, 0);
	if (fcn) {
