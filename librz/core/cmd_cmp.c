...
RZ_IPI int rz_cmd_cmp(void *data, const char *input) {
	...
	case 'X':
		...
		if (buf) {
			if (!rz_io_read_at (core->io, rz_num_math (core->num,
	...
	{          // XXX: this is broken
		...
		case 'o':         // "cgo"
			...
			rz_analysis_diff_setup (core->anal, true, -1, -1);
			break;
		case 'f':         // "cgf"
		...
		case ' ':
			file2 = (char *) rz_str_trim_head_ro (input + 2);
			rz_analysis_diff_setup (core->anal, false, -1, -1);
			break;
		default: {
			const char *help_message[] = {
		...
		rz_analysis_diff_setup (core->anal, diffops, -1, -1);
		rz_analysis_diff_setup (core2->anal, diffops, -1, -1);
