...
RZ_API RzCoreItem *rz_core_item_at (RzCore *core, ut64 addr) {
	...
	if (map) {
		...
		if (map->perm & RZ_PERM_X) {
			...
			ut64 size;
			RzAnalysisMetaItem *item = rz_meta_get_at (core->anal, addr, RZ_META_TYPE_ANY, &size);
			if (item) {
				switch (item->type) {
	...
	if (!ci->data) {
		RzAnalysisOp* op = rz_core_anal_op (core, addr, RZ_ANAL_OP_MASK_ESIL | RZ_ANAL_OP_MASK_HINT);
		if (op) {
			if (!ci->data) {
				if (op->mnemonic) {
