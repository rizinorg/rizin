...
/**
 * \brief Layout-pass cancellation check.
 *
 * Routes through the RzGraphLayoutConfig callback so the layout algorithm does
 * not depend on RzCons directly (the seam towards moving this code into
 * RzUtil, rizinorg/rizin#992). Falls back to rz_cons_is_breaked() when no
 * callback is configured, preserving the historic behaviour.
 */
static inline bool agraph_layout_cancelled(const RzAGraph *g) {
	if (g->layout_config.is_cancelled) {
		return g->layout_config.is_cancelled(g->layout_config.user);
	}
