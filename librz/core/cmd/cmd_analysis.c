...
/**
 * \brief analyze all arch preludes in static arch database and create functions on hit
 */
RZ_IPI RzCmdStatus rz_analyze_all_preludes_handler(RzCore *core, int argc, const char **argv) {
	rz_core_search_preludes(core, NULL);
	return RZ_CMD_STATUS_OK;
}

/**
 * \brief Load prologues signatures from the json file (generated by prologues generator plugin) and analyse current binary
 */
RZ_IPI RzCmdStatus rz_load_and_analyze_all_preludes_handler(RzCore *core, int argc, const char **argv) {
	...
	// validate arch metadata from file against curr open binary
	...
	for (ele = prologues_js->children.first; ele; ele = ele->next) {
		...
		// OR just do whatever RzSearchBytesPattern does
		...
		RzSearchKeyword *kw = rz_search_keyword_new(bytes_buf, expected_len, mask_buf, expected_len, NULL);
		RZ_FREE(bytes_buf);
		RZ_FREE(mask_buf);
		if (!kw) {
			RZ_LOG_ERROR("Failed to create RzSearchKeyword for prologue\n");
			goto cleanup;
		}
...
/**
 * \brief Search for a prologue keyword (byte+mask) and create new functions on hit
 */
RZ_IPI RzCmdStatus rz_analyze_all_preludes_keyword_handler(RzCore *core, int argc, const char **argv) {
	const char *byte_pattern = argv[1];
	RzSearchBytesPattern *pattern = rz_search_parse_byte_pattern(byte_pattern, NULL);
	if (!pattern) {
		RZ_LOG_ERROR("Invalid byte pattern: %s\n", byte_pattern);
		return RZ_CMD_STATUS_ERROR;
	}
	size_t len = rz_search_bytes_pattern_len(pattern);
