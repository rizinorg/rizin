...
static int myregwrite(RzAnalysisEsil *esil, const char *name, ut64 *val) {
	...
	if (*val) {
		...
		char *type = NULL;
		(void)rz_io_read_at_mapped(esil->analysis->iob.io, *val, (ut8 *)str, sizeof(str) - 1);
		str[sizeof(str) - 1] = 0;
		ds->emuptr = *val;
		// support cstring here
		{
			...
			if (mem_0 == 0 && mem_1 < 0x1000) {
				...
				(void)rz_io_read_at_mapped(esil->analysis->iob.io, addr,
					(ut8 *)str, sizeof(str) - 1);
...
RZ_IPI int rz_core_print_disasm_all(RzCore *core, ut64 addr, int l, int len) {
	...
	for (i = 0; i < l; i++) {
		...
		if (ret < 1) {
			RZ_LOG_ERROR("Failed to print disassembly at the address 0x%08" PFMT64x, ds->vat);
		} else {
			count++;
