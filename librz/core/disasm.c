...
static char* colorize_asm_string(RzCore* core, RzDisasmState* ds, bool print_color)
{
    ...
    if (spacer)
    {
        ...
        scol1 = rz_print_colorize_opcode(ds->core->print, s1, ds->color_reg, ds->color_num, partial_reset, f ? f->addr
                                                                                                             : 0);
        free(s1);
        scol2 = rz_print_colorize_opcode(ds->core->print, s2, ds->color_reg, ds->color_num, partial_reset, f ? f->addr
                                                                                                             : 0);
        free(s2);
        if (!scol1)
        {
    ...
    return rz_print_colorize_opcode(ds->core->print, source, ds->color_reg, ds->color_num, partial_reset, f ? f->addr
                                                                                                            : 0);
}
...
static void ds_build_op_str(RzDisasmState* ds, bool print_color)
{
    ...
    if (core->parser->subrel
        && (ds->analysis_op.type == RZ_ANALYSIS_OP_TYPE_LEA || ds->analysis_op.type == RZ_ANALYSIS_OP_TYPE_MOV
            || ds->analysis_op.type == RZ_ANALYSIS_OP_TYPE_CMP) && ds->analysis_op.ptr != UT64_MAX)
    {
    ...
    if (ds->subvar && ds->opstr)
    {
        ...
        if (core->parser->subrel)
        {
            ...
            rz_list_foreach (list, iter, xref)
                {
                    if ((xref->type == RZ_ANALYSIS_XREF_TYPE_DATA || xref->type == RZ_ANALYSIS_XREF_TYPE_STRING)
                        && ds->analysis_op.type == RZ_ANALYSIS_OP_TYPE_LEA)
                    {
...
static void ds_setup_pre(RzDisasmState* ds, bool tail, bool middle)
{
    ...
    if (f)
    {
        ...
        else if (rz_analysis_function_max_addr(f) - ds->analysis_op.size == ds->at
                 && f->addr == rz_analysis_function_min_addr(f))
        {
            ds->pre = DS_PRE_FCN_TAIL;
        }
...
static void ds_print_fcn_name(RzDisasmState* ds)
{
    ...
    if (ds->analysis_op.type != RZ_ANALYSIS_OP_TYPE_JMP && ds->analysis_op.type != RZ_ANALYSIS_OP_TYPE_CJMP
        && ds->analysis_op.type != RZ_ANALYSIS_OP_TYPE_CALL)
    {
        return;
    }
    ...
    if (!f && ds->core->flags && (!ds->core->vmode || (!ds->subjmp && !ds->subnames)))
    {
            ...
            && (arch = rz_config_get(ds->core->config, "asm.arch")) && strcmp(arch, "dalvik"))
        {
            ...
            if (ds->core->vmode && ds->asm_demangle && (rz_str_startswith(flag->name, "sym.")
                                                        || (flag_sym = rz_flag_get_by_spaces(ds->core->flags, ds->analysis_op.jump, RZ_FLAGS_FS_SYMBOLS, NULL)))
                && flag_sym->demangled)
            {
                return;
            }
...
static void ds_print_ptr(RzDisasmState* ds, int len, int idx)
{
    ...
    rz_list_free(list);
    if (ds->analysis_op.type == (RZ_ANALYSIS_OP_TYPE_MOV | RZ_ANALYSIS_OP_TYPE_REG)
        && ds->analysis_op.stackop == RZ_ANALYSIS_STACK_SET && ds->analysis_op.val != UT64_MAX
        && ds->analysis_op.val > 10)
    {
    ...
    else if (((st64)p) > 0 || ((st64)refaddr) > 0)
    {
        ...
        if (((st64)p) > 0)
        {
            ...
            if (f)
            {
                ...
                if (!flag_printed && !is_filtered_flag(ds, f->name)
                    && (!ds->opstr || (!strstr(ds->opstr, f->name) && !strstr(ds->opstr, f->realname))) && !f2_in_opstr)
                {
                    ds_begin_comment(ds);
                    ds_comment(ds, true, "; %s", f->name);
        ...
        bool print_msg = true;
#if 1
        if (ds->strenc == RZ_STRING_ENC_GUESS && rz_utf_bom_encoding((ut8*)msg, len) == RZ_STRING_ENC_GUESS
...
static void ds_print_comments_right(RzDisasmState* ds)
{
    ...
    free(desc);
    if ((ds->analysis_op.type == RZ_ANALYSIS_OP_TYPE_CALL || ds->analysis_op.type & RZ_ANALYSIS_OP_TYPE_UCALL)
        && ds->show_calls)
    {
...
toro:
    ...
    core->cons->vline = rz_config_get_b(core->config, "scr.utf8") ? (rz_config_get_b(core->config, "scr.utf8.curvy")
                                                                     ? rz_vline_uc : rz_vline_u) : rz_vline_a;
        ...
        else
        {
            ...
            else
            {
                ...
                if (show_color)
                {
                    ...
                    asm_str = rz_print_colorize_opcode(core->print, asm_str, color_reg, color_num, false, f ? f->addr
                                                                                                            : 0);
                    rz_cons_printf("%s%s" Color_RESET "\n",
