...
static void rtr_textlog_chat (RzCore *core, TextLog T) {
	char prompt[64];
	char buf[1024];
	int lastmsg = 0;
	...
	char *oldprompt = strdup (rz_line_singleton ()->prompt);
	...
	for (;;) {
		...
		if (!strcmp (buf, "/help")) {
			...
			char *cmd = rz_str_newf ("T [%s] %s", me, buf);
			free (rtrcmd (T, cmd));
			free (cmd);
		}
	}
beach:
	rz_line_set_prompt (oldprompt);
	free (oldprompt);
}
...
#endif
	if (((size_t)u) > 0xff) {
		port = listenport? listenport: rz_config_get (
...
static void rtr_cmds_read(uv_stream_t *client, ssize_t nread, const uv_buf_t *buf) {
	...
	*end = '\0';
	...
	if (!client_context->res || (!rz_config_get_i (client_context->core->config, "scr.prompt") &&
				 !strcmp ((char *)buf, "q!")) ||
				 !strcmp ((char *)buf, ".--")) {
		rtr_cmds_client_close ((uv_tcp_t *) client, true);
		return;
