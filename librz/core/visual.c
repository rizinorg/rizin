...
static const char *__core_visual_print_command(RzCore *core) {
	...
	if (rz_config_get_i (core->config, "scr.dumpcols")) {
		free (core->stkcmd);
		core->stkcmd = rz_str_newf (stackPrintCommand (core));
...
static void rotateAsmBits(RzCore *core) {
	RzAnalHint *hint = rz_anal_hint_get (core->anal, core->offset);
...
static void findPair(RzCore *core) {
	...
	if (p && (delta % 2)) {
		...
		q = rz_mem_mem (core->block + d, core->blocksize - d,
			(const ut8 *) buf, len);
		if (!q) {
			q = rz_mem_mem (core->block, R_MIN (core->blocksize, d),
				(const ut8 *) buf, len);
		}
	}
	if (q) {
...
static void setprintmode(RzCore *core, int n) {
	...
	case R_CORE_VISUAL_MODE_DB:
		rz_asm_op_init (&op);
		rz_asm_disassemble (core->rasm, &op, core->block, R_MIN (32, core->blocksize));
		rz_asm_op_fini (&op);
		break;
	default:
		break;
...
static ut64 prevop_addr(RzCore *core, ut64 addr) {
	...
	int maxop = rz_anal_archinfo (core->anal, R_ANAL_ARCHINFO_MAX_OP_SIZE);
...
static int follow_ref(RzCore *core, RzList *xrefs, int choice, int xref) {
	RzAnalRef *refi = rz_list_get_n (xrefs, choice);
	if (refi) {
		if (core->print->cur_enabled) {
			core->print->cur = 0;
		}
...
repeat:
	rz_list_free (xrefs);
	if (xrefsMode) {
		RzAnalFunction *fun = rz_anal_get_fcn_in (core->anal, addr, R_ANAL_FCN_TYPE_NULL);
		if (fun) {
	...
	if (!xrefs || rz_list_empty (xrefs)) {
		...
		rz_list_foreach (xrefs, iter, refi) {
			...
			if (idx >= skip) {
				...
				RzAnalFunction *fun = rz_anal_get_fcn_in (core->anal, refi->addr, R_ANAL_FCN_TYPE_NULL);
				char *name;
				if (fun) {
...
char *getcommapath(RzCore *core) {
	...
	if (dir && *dir && prj && *prj) {
		...
		cwd = rz_str_newf ("%s"R_SYS_DIR "%s.d", abspath, prj);
		free (abspath);
	} else {
...
static void visual_comma(RzCore *core) {
	...
	const char *prev_cmt = rz_meta_get_string (core->anal, R_META_TYPE_COMMENT, addr);
	...
	if (!cmtfile) {
		...
		if (fn && *fn) {
			...
			if (!comment || !*comment) {
				...
				rz_meta_set_string (core->anal, R_META_TYPE_COMMENT, addr, comment);
			} else {
	...
	if (cmtfile) {
		char *cwf = rz_str_newf ("%s"R_SYS_DIR "%s", cwd, cmtfile);
...
static void nextOpcode(RzCore *core) {
	RzAnalOp *aop = rz_core_anal_op (core, core->offset + core->print->cur, R_ANAL_OP_MASK_BASIC);
	RPrint *p = core->print;
	if (aop) {
		p->cur += aop->size;
		rz_anal_op_free (aop);
...
RZ_API int rz_core_visual_cmd(RzCore *core, const char *arg) {
	...
	if (isNumber (core, ch)) {
		...
		if (isDisasmPrint (core->printidx)) {
			if (rz_config_get_i (core->config, "asm.hints") && (rz_config_get_i (core->config, "asm.hint.jmp")
			|| rz_config_get_i (core->config, "asm.hint.lea") || rz_config_get_i (core->config, "asm.hint.emu")
			|| rz_config_get_i (core->config, "asm.hint.call"))) {
		...
		case 0x0d: // "enter" "\\n" "newline"
		{
			...
			do {
				op = rz_core_anal_op (core, core->offset + core->print->cur, R_ANAL_OP_MASK_BASIC);
				if (op) {
		...
		case 'j':
			if (core->print->cur_enabled) {
				...
				if (rz_config_get_i (core->config, "scr.wheel.nkey")) {
					...
					ut64 amisize;
					RzAnalMetaItem *ami = rz_meta_get_at (core->anal, core->offset, R_META_TYPE_DATA, &amisize);
					if (!ami) {
						ami = rz_meta_get_at (core->anal, core->offset, R_META_TYPE_STRING, &amisize);
					}
					if (ami) {
		...
		case 'm':
			{
				rz_cons_gotoxy (0, 0);
				rz_cons_printf (R_CONS_CLEAR_LINE"Set shortcut key for 0x%"PFMT64x"\n", core->offset);
				rz_cons_flush ();
				int ch = rz_cons_readchar ();
		...
		case 'y':
			if (core->print->ocur == -1) {
				...
				rz_core_yank (core, core->offset + ((core->print->ocur < core->print->cur) ?
...
RZ_API void rz_core_visual_title(RzCore *core, int color) {
	...
	RzIODesc *desc = map
		...
		: core->file? rz_io_desc_get (core->io, core->file->fd): NULL;
	...
	{ /* get flag with delta */
		...
		RzFlagItem *f = NULL;
		if (rz_flag_space_push (core->flags, R_FLAGS_FS_SYMBOLS)) {
...
RZ_API void rz_core_visual_disasm_up(RzCore *core, int *cols) {
	RzAnalFunction *f = rz_anal_get_fcn_in (core->anal, core->offset, R_ANAL_FCN_TYPE_NULL);
	if (f && f->folded) {
...
RZ_API RzListInfo *rz_listinfo_new(const char *name, RInterval pitv, RInterval vitv, int perm, const char *extra) {
	RzListInfo *info = R_NEW (RzListInfo);
	if (info) {
