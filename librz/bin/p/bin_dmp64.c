...
static void header(RzBinFile *bf) {
	...
	if (obj->bmp_header) {
		...
		rbin->cb_printf("  BStoreOffset : 0x%08" PFMT32x "\n", rz_read_le64(&obj->triage64_header->ArchitectureSpecific.Ia64.BStoreOffset));
		rbin->cb_printf("  SizeOfBStore : 0x%08" PFMT32x "\n", rz_read_le64(&obj->triage64_header->ArchitectureSpecific.Ia64.SizeOfBStore));
...
static RzList *fields(RzBinFile *bf) {
	RzList *fields = rz_list_newf(rz_bin_field_free);
	struct rz_bin_dmp64_obj_t *obj = (struct rz_bin_dmp64_obj_t *)bf->o->bin_obj;
	struct rz_bin_t *rbin = bf->rbin;
