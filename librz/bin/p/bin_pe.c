...
static RzStructFactory *pe_structure(RzBinFile *bf) {
	...
	RzStructFactory *pe32 = rz_struct_factory_map_add_map(info, "pe32");
	if (!pe32) {
		rz_struct_factory_free(info);
		return NULL;
	}

	RzStructFactory *image = rz_struct_factory_map_add_map(pe32, "IMAGE_NT_HEADERS");
	if (!image) {
		rz_struct_factory_free(info);
		return NULL;
	}
	...
	if (!(image = rz_struct_factory_map_add_map(pe32, "IMAGE_FILE_HEADERS"))) {
		rz_struct_factory_free(info);
		return NULL;
	}
	...
	if (!(image = rz_struct_factory_map_add_map(pe32, "IMAGE_OPTIONAL_HEADERS"))) {
		rz_struct_factory_free(info);
		return NULL;
	}
	...
	rz_struct_factory_map_add_unsigned(image, "BaseOfData", bin->nt_headers->optional_header.BaseOfData, true);
	...
	if (rz_list_length(bin->rich_entries) > 0) {
		if (!(image = rz_struct_factory_map_add_array(pe32, "RICH_FIELDS"))) {
			rz_struct_factory_free(info);
			return NULL;
		}
	...
	for (size_t i = 0; i < PE_IMAGE_DIRECTORY_ENTRIES - 1; i++) {
		...
		if (!(image = rz_struct_factory_map_add_map(pe32, name))) {
			rz_struct_factory_free(info);
			return NULL;
		}
