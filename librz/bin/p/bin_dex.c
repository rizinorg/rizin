...
static void dex_parse_debug_item(RBinFile *bf, RBinDexClass *c, int MI, int MA, int paddr, int ins_size, int insns_size, char *class_name, int regsz, int debug_info_off) {
	...
	rz_buf_seek (bf->buf, debug_info_off, R_BUF_SET);
	ut64 res;
		...
		if (MC > 0) {
			...
			if (tries_size > 0) {
				...
				for (j = 0; j < tries_size; j++) {
					...
					st64 size;
					if (rz_buf_seek (bf->buf, off, R_BUF_SET) == -1) {
						break;
					}
				...
				rz_buf_seek (bf->buf, offorig, R_BUF_SET);
			} else {
				if (dexdump) {
					cb_printf (
						"      catches       : "
		...
		if (*flag_name) {
			...
			if (MC > 0 && debug_info_off > 0 && dex->header.data_offset < debug_info_off &&
				...
				if (bin_dbginfo) {
					...
					rz_buf_seek (bf->buf, addr, R_BUF_SET);
				}
			} else if (MC > 0) {
				if (dexdump) {
					cb_printf ("      positions     :\n");
...
static void parse_class(RBinFile *bf, RBinDexClass *c, int class_index, int *methods, int *sym_count) {
	...
	if (!c || !c->class_data_offset) {
		...
		bool err = false;
		size_t skip = 0;
		rz_buf_seek (bf->buf, c->class_data_offset, R_BUF_SET);
...
#define ROW(nam,siz,val,fmt) \
	...
	rz_buf_seek (bf->buf, 0, R_BUF_SET);
...
static RzList* libs(RBinFile *bf) {
	...
	rz_list_foreach (files, iter, file) {
		if (is_classes_dex (file)) {
			char *n = rz_str_newf ("%s%s%s", path, R_SYS_DIR, file);
			if (strcmp (n, bf->file)) {
