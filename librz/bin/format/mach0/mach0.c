...
static bool parse_dysymtab(struct MACH0_(obj_t) *bin, ut64 off) {
	...
	if (bin->ntoc > 0) {
		...
		for (i = 0; i < bin->ntoc; i++) {
			len = rz_buf_read_at (bin->b, bin->dysymtab.tocoff +
	...
	if (bin->nmodtab > 0) {
		...
		for (i = 0; i < bin->nmodtab; i++) {
			len = rz_buf_read_at (bin->b, bin->dysymtab.modtaboff +
	...
	for (i = 0; i < super.count; i++) {
		...
		case CSSLOT_CMS_SIGNATURE: // ASN1/DER certificate
			if (isVerbose) {
				...
				ut32 length = R_MIN (UT16_MAX, rz_read_ble32 (header + 4, 1));
...
static bool parse_chained_fixups(struct MACH0_(obj_t) *bin, ut32 offset, ut32 size) {
	...
	bin->chained_starts = R_NEWS0 (struct rz_dyld_chained_starts_in_segment *, segs_count);
	...
	for (i = 0; i < segs_count; i++) {
		...
		struct rz_dyld_chained_starts_in_segment *cur_seg = R_NEW0 (struct rz_dyld_chained_starts_in_segment);
		if (!cur_seg) {
			return false;
		}
		...
		if (cur_seg->page_count > 0) {
			...
			if (rz_buf_fread_at (bin->b, starts_at + seg_off + 22, (ut8 *)page_start, "s", cur_seg->page_count)
...
static bool reconstruct_chained_fixup(struct MACH0_(obj_t) *bin) {
	...
	bin->chained_starts = R_NEWS0 (struct rz_dyld_chained_starts_in_segment *, bin->nsegs);
	if (!bin->chained_starts) {
		return false;
	}
		...
		case BIND_OPCODE_THREADED: {
			...
			case BIND_SUBOPCODE_THREADED_APPLY: {
				...
				if (!cur_seg || cur_seg_idx != seg_idx) {
					...
					if (!cur_seg) {
						cur_seg = R_NEW0 (struct rz_dyld_chained_starts_in_segment);
						if (!cur_seg) {
							break;
						}
...
RzList *MACH0_(get_segments)(RBinFile *bf) {
	...
	if (bin->nsects > 0) {
		...
		for (i = 0; i < last_section; i++) {
			...
			char *segment_name = rz_str_newf ("%d.%s", i, bin->segs[segment_index].segname);
...
struct import_t *MACH0_(get_imports)(struct MACH0_(obj_t) *bin) {
	...
	for (i = j = 0; i < bin->dysymtab.nundefsym; i++) {
		...
		if (imp_name) {
			rz_str_ncpy (imports[j].name, imp_name, R_BIN_MACH0_STRING_LENGTH);
			free (imp_name);
		} else {
			//imports[j].name[0] = 0;
			continue;
...
#define SLEB() rz_sleb128 ((const ut8 **)&p, end)
				case BIND_OPCODE_DONE: {
