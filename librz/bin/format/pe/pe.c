...
static int bin_pe_parse_imports(struct PE_(rz_bin_pe_obj_t)* bin,
	...
	do {
		...
		len = rz_buf_read_at (bin->b, off + i * sizeof (PE_DWord), (ut8*) &import_table, sizeof (PE_DWord));
		if (len != sizeof (PE_DWord)) {
			...
			if (import_table & ILT_MASK1) {
				...
				len = rz_buf_read_at (bin->b, off, (ut8*) &import_hint, sizeof (PE_Word));
...
static int bin_pe_init_hdr(struct PE_(rz_bin_pe_obj_t)* bin) {
	...
	if (rz_buf_read_at (bin->b, 0, (ut8*) bin->dos_header, sizeof(PE_(image_dos_header))) < 0) {
	...
	if (rz_buf_read_at (bin->b, bin->dos_header->e_lfanew, (ut8*) bin->nt_headers, sizeof (PE_(image_nt_headers))) < 0) {
		bprintf ("Warning: read (dos header)\n");
		return false;
	}
...
static int bin_pe_init_sections(struct PE_(rz_bin_pe_obj_t)* bin) {
	...
	if (rz_buf_read_at (bin->b, bin->section_header_offset, (ut8*) bin->section_header, sections_size) < 0) {
...
static int bin_pe_init_clr_hdr(struct PE_(rz_bin_pe_obj_t)* bin) {
	...
	rr = rz_buf_read_at (bin->b, image_clr_hdr_paddr, (ut8*) (clr_hdr), len);
...
static int bin_pe_init_imports(struct PE_(rz_bin_pe_obj_t)* bin) {
	...
	if (import_dir_paddr != 0) {
		...
		do {
			...
			if (rz_buf_read_at (bin->b, import_dir_offset + indx * dir_size, (ut8*) (curr_import_dir), dir_size) <= 0) {
	...
	if (rz_buf_size (bin->b) > 0) {
		if ((delay_import_dir_offset != 0) && (delay_import_dir_offset < (ut32)rz_buf_size (bin->b))) {
			...
			do {
				...
				rr = rz_buf_read_at (bin->b, delay_import_dir_offset + (indx - 1) * delay_import_size,
...
static int bin_pe_init_exports(struct PE_(rz_bin_pe_obj_t)* bin) {
	...
	if (rz_buf_read_at (bin->b, export_dir_paddr, (ut8*) bin->export_directory, sizeof (PE_(image_export_directory))) < 0) {
...
static int bin_pe_init_resource(struct PE_(rz_bin_pe_obj_t)* bin) {
	...
	if (rz_buf_read_at (bin->b, resource_dir_paddr, (ut8*) bin->resource_directory,
...
static void bin_pe_store_tls_callbacks(struct PE_(rz_bin_pe_obj_t)* bin, PE_DWord callbacks) {
	...
	while (addressOfTLSCallback != 0) {
		if (rz_buf_read_at (bin->b, callbacks, (ut8*) &addressOfTLSCallback, sizeof(addressOfTLSCallback)) != sizeof (addressOfTLSCallback)) {
...
static int bin_pe_init_tls(struct PE_(rz_bin_pe_obj_t)* bin) {
	...
	if (rz_buf_read_at (bin->b, tls_paddr, (ut8*) image_tls_directory, sizeof (PE_(image_tls_directory))) != sizeof (PE_(image_tls_directory))) {
...
static Var* Pe_r_bin_pe_parse_var(struct PE_(rz_bin_pe_obj_t)* bin, PE_DWord* curAddr) {
	...
	if (rz_buf_read_at (bin->b, *curAddr, (ut8*) &var->wLength, sizeof(var->wLength)) != sizeof(var->wLength)) {
		bprintf ("Warning: read (Var wLength)\n");
		free_Var (var);
		return NULL;
	}
	*curAddr += sizeof(var->wLength);
	if (rz_buf_read_at (bin->b, *curAddr, (ut8*) &var->wValueLength, sizeof(var->wValueLength)) != sizeof(var->wValueLength)) {
		bprintf ("Warning: read (Var wValueLength)\n");
		free_Var (var);
		return NULL;
	}
	*curAddr += sizeof(var->wValueLength);
	if (rz_buf_read_at (bin->b, *curAddr, (ut8*) &var->wType, sizeof(var->wType)) != sizeof(var->wType)) {
...
static VarFileInfo* Pe_r_bin_pe_parse_var_file_info(struct PE_(rz_bin_pe_obj_t)* bin, PE_DWord* curAddr) {
	...
	if (rz_buf_read_at (bin->b, *curAddr, (ut8*) &varFileInfo->wLength, sizeof(varFileInfo->wLength)) != sizeof(varFileInfo->wLength)) {
	...
	*curAddr += sizeof(varFileInfo->wLength);
	...
	if (rz_buf_read_at (bin->b, *curAddr, (ut8*) &varFileInfo->wValueLength, sizeof(varFileInfo->wValueLength)) != sizeof(varFileInfo->wValueLength)) {
	...
	*curAddr += sizeof(varFileInfo->wValueLength);
	...
	if (rz_buf_read_at (bin->b, *curAddr, (ut8*) &varFileInfo->wType, sizeof(varFileInfo->wType)) != sizeof(varFileInfo->wType)) {
...
static String* Pe_r_bin_pe_parse_string(struct PE_(rz_bin_pe_obj_t)* bin, PE_DWord* curAddr) {
	...
	if (rz_buf_read_at (bin->b, *curAddr, (ut8*) &string->wLength, sizeof(string->wLength)) != sizeof(string->wLength)) {
	...
	*curAddr += sizeof(string->wLength);
	...
	if (rz_buf_read_at (bin->b, *curAddr, (ut8*) &string->wValueLength, sizeof(string->wValueLength)) != sizeof(string->wValueLength)) {
	...
	*curAddr += sizeof(string->wValueLength);
	...
	if (rz_buf_read_at (bin->b, *curAddr, (ut8*) &string->wType, sizeof(string->wType)) != sizeof(string->wType)) {
...
static StringTable* Pe_r_bin_pe_parse_string_table(struct PE_(rz_bin_pe_obj_t)* bin, PE_DWord* curAddr) {
	...
	PE_DWord startAddr = *curAddr;
	if (rz_buf_read_at (bin->b, *curAddr, (ut8*) &stringTable->wLength, sizeof(stringTable->wLength)) != sizeof(stringTable->wLength)) {
	...
	*curAddr += sizeof(stringTable->wLength);
	...
	if (rz_buf_read_at (bin->b, *curAddr, (ut8*) &stringTable->wValueLength, sizeof(stringTable->wValueLength)) != sizeof(stringTable->wValueLength)) {
	...
	*curAddr += sizeof(stringTable->wValueLength);
	...
	if (rz_buf_read_at (bin->b, *curAddr, (ut8*) &stringTable->wType, sizeof(stringTable->wType)) != sizeof(stringTable->wType)) {
...
static StringFileInfo* Pe_r_bin_pe_parse_string_file_info(struct PE_(rz_bin_pe_obj_t)* bin, PE_DWord* curAddr) {
	...
	if (rz_buf_read_at (bin->b, *curAddr, (ut8*) &stringFileInfo->wLength, sizeof(stringFileInfo->wLength)) != sizeof(stringFileInfo->wLength)) {
	...
	*curAddr += sizeof(stringFileInfo->wLength);
	...
	if (rz_buf_read_at (bin->b, *curAddr, (ut8*) &stringFileInfo->wValueLength, sizeof(stringFileInfo->wValueLength)) != sizeof(stringFileInfo->wValueLength)) {
	...
	*curAddr += sizeof(stringFileInfo->wValueLength);
	...
	if (rz_buf_read_at (bin->b, *curAddr, (ut8*) &stringFileInfo->wType, sizeof(stringFileInfo->wType)) != sizeof(stringFileInfo->wType)) {
...
static PE_VS_VERSIONINFO* Pe_r_bin_pe_parse_version_info(struct PE_(rz_bin_pe_obj_t)* bin, PE_DWord version_info_paddr) {
	...
	sz = sizeof(ut16);
	EXIT_ON_OVERFLOW (sz);
	if (rz_buf_read_at (bin->b, curAddr, (ut8*) &vs_VersionInfo->wLength, sz) != sz) {
		bprintf ("Warning: read (VS_VERSIONINFO wLength)\n");
		goto out_error;
	}
	curAddr += sz;
	EXIT_ON_OVERFLOW (sz);
	if (rz_buf_read_at (bin->b, curAddr, (ut8*) &vs_VersionInfo->wValueLength, sz) != sz) {
		bprintf ("Warning: read (VS_VERSIONINFO wValueLength)\n");
		goto out_error;
	}
	curAddr += sz;
	EXIT_ON_OVERFLOW (sz);
	if (rz_buf_read_at (bin->b, curAddr, (ut8*) &vs_VersionInfo->wType, sz) != sz) {
...
static void _parse_resource_directory(struct PE_(rz_bin_pe_obj_t) *bin, Pe_image_resource_directory *dir, ut64 offDir, int type, int id, HtUU *dirs, const char *resource_name) {
	...
	for (index = 0; index < totalRes; index++) {
		...
		if (rz_buf_read_at (bin->b, off, (ut8*)&entry, sizeof(entry)) < 1) {
		...
		if (entry.u1.s.NameIsString) {
			int i;
			ut16 buf;
			if (rz_buf_read_at (bin->b, bin->resource_directory_offset + entry.u1.s.NameOffset, (ut8*)&buf, sizeof (ut16)) != sizeof (ut16)) {
				break;
			}
			...
			if (resourceEntryName) {
				for (i = 0; i < resourceEntryNameLength; i++) { /* Convert Unicode to ASCII */
					ut8 byte;
					int r = rz_buf_read_at (bin->b, bin->resource_directory_offset + entry.u1.s.NameOffset + 2 + (i*2), &byte, sizeof (ut8));
		...
		if (entry.u2.s.DataIsDirectory) {
			...
			int len = rz_buf_read_at (bin->b, off, (ut8*) &identEntry, sizeof (identEntry));
		...
		if (rz_buf_read_at (bin->b, off, (ut8*)data, sizeof (*data)) != sizeof (*data)) {
		...
		rz_pe_resource *rs = R_NEW0 (rz_pe_resource);
		if (!rs) {
			free (data);
			break;
		}
...
RZ_API void PE_(bin_pe_parse_resource)(struct PE_(rz_bin_pe_obj_t) *bin) {
	...
	for (index = 0; index < totalRes; index++) {
		...
		if (rz_buf_read_at (bin->b, off, (ut8*)&typeEntry, sizeof(typeEntry)) < 1) {
			...
			if (bin->export_directory->NumberOfNames > 0) {
				...
				for (n = 0; n < bin->export_directory->NumberOfNames; n++) {
					...
					if (i == fo) {
						...
						rz_buf_read_at (bin->b, names_paddr + n * sizeof (PE_VWord), (ut8*) &name_vaddr, sizeof (PE_VWord));
						break;
					}
				}
...
int PE_(rz_bin_pe_get_debug_data)(struct PE_(rz_bin_pe_obj_t)* bin, SDebugInfo* res) {
	...
	rz_buf_read_at (bin->b, dbg_dir_offset, (ut8 *)&img_dbg_dir_entry, sizeof (img_dbg_dir_entry));
	if ((rz_buf_size (bin->b) - dbg_dir_offset) < sizeof (PE_(image_debug_directory_entry))) {
		return false;
	}
	ut32 dbg_data_poff = R_MIN (img_dbg_dir_entry.PointerToRawData, rz_buf_size (bin->b));
	int dbg_data_len = R_MIN (img_dbg_dir_entry.SizeOfData, rz_buf_size (bin->b) - dbg_data_poff);
	if (dbg_data_len < 1) {
		return false;
	}
...
struct rz_bin_pe_import_t* PE_(rz_bin_pe_get_imports)(struct PE_(rz_bin_pe_obj_t)* bin) {
	...
	if (off < bin->size && off > 0) {
		...
		int r = rz_buf_read_at (bin->b, bin->import_directory_offset + idi * sizeof (curr_import_dir),
		...
		while (r == sizeof (curr_import_dir) && bin->import_directory_offset + (idi + 1) * sizeof (curr_import_dir) <= last && (
			...
			idi++;
			r = rz_buf_read_at (bin->b, bin->import_directory_offset + idi * sizeof (curr_import_dir),
				(ut8 *)&curr_import_dir, sizeof (curr_import_dir));
			if (r < 0) {
				free (imports);
				return NULL;
	...
	if (off < bin->size && off > 0) {
		...
		int r = rz_buf_read_at (bin->b, off + didi * sizeof (curr_delay_import_dir),
		...
		while ((curr_delay_import_dir.Name != 0) && (curr_delay_import_dir.DelayImportAddressTable !=0)) {
			...
			didi++;
			r = rz_buf_read_at (bin->b, off + didi * sizeof (curr_delay_import_dir),
				(ut8 *)&curr_delay_import_dir, sizeof (curr_delay_import_dir));
			if (r != sizeof (curr_delay_import_dir)) {
				goto beach;
			}
...
struct rz_bin_pe_lib_t* PE_(rz_bin_pe_get_libs)(struct PE_(rz_bin_pe_obj_t)* bin) {
	...
	if (off < bin->size && off > 0) {
		...
		int r = rz_buf_read_at (bin->b, off + iidi * sizeof (curr_import_dir),
			(ut8 *)&curr_import_dir, sizeof (curr_import_dir));
...
next:
			iidi++;
			r = rz_buf_read_at (bin->b, off + iidi * sizeof (curr_import_dir),
				(ut8 *)&curr_import_dir, sizeof (curr_import_dir));
		}
	}
	off = bin->delay_import_directory_offset;
	if (off < bin->size && off > 0) {
		...
		int r = rz_buf_read_at (bin->b, off, (ut8 *)&curr_delay_import_dir, sizeof (curr_delay_import_dir));
		if (r != sizeof (curr_delay_import_dir)) {
			goto out_error;
		}
		while (r == sizeof (curr_delay_import_dir) &&
			...
			did++;
			r = rz_buf_read_at (bin->b, off + did * sizeof (curr_delay_import_dir),
				(ut8 *)&curr_delay_import_dir, sizeof (curr_delay_import_dir));
		}
	}
	sdb_ht_free (lib_map);
...
struct PE_(rz_bin_pe_obj_t)* PE_(rz_bin_pe_new)(const char* file, bool verbose) {
	struct PE_(rz_bin_pe_obj_t)* bin = R_NEW0 (struct PE_(rz_bin_pe_obj_t));
	if (!bin) {
		return NULL;
	}
	bin->file = file;
