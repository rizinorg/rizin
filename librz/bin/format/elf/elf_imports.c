...
static bool convert_elf_symbol_to_elf_import(ELFOBJ *bin, RzBinElfSymbol *import, RzBinElfSymbol *symbol) {
	memcpy(import, symbol, sizeof(RzBinElfSymbol));
...
static RzVector *get_elf_imports(ELFOBJ *bin) {
	...
	rz_bin_elf_foreach_elf_import_symbols_as_elf_symbols(bin, symbol) {
...
static RzVector *get_imports(RzVector *elf_imports) {
	RzVector *result = rz_vector_new(sizeof(RzBinImport), NULL, NULL);
	if (!result) {
		return NULL;
	}
...
static RzVector *get_symbols(ELFOBJ *bin, RzVector *elf_imports) {
	RzVector *result = Elf_(rz_bin_elf_convert_symbols)(bin, elf_imports);
	if (!result) {
		return NULL;
	}

	RzBinSymbol *symbol;
	rz_vector_foreach(result, symbol) {
...
RZ_OWN RzBinElfImports *Elf_(rz_bin_elf_imports_new)(RZ_NONNULL ELFOBJ *bin) {
	...
	RzBinElfImports *result = RZ_NEW0(RzBinElfImports);
	if (!result) {
		return NULL;
	}

	RzVector *elf_imports = get_elf_imports(bin);
	if (!elf_imports) {
		Elf_(rz_bin_elf_imports_free)(result);
		return NULL;
	}

	result->symbols = get_symbols(bin, elf_imports);
	if (!result->symbols) {
		rz_vector_free(elf_imports);
		Elf_(rz_bin_elf_imports_free)(result);
		return NULL;
	}

	result->imports = get_imports(elf_imports);
