...
static bool rz_bin_mdmp_init_hdr(struct rz_bin_mdmp_obj *obj) {
	...
	rz_buf_read_at (obj->b, 0, (ut8 *)obj->hdr, sizeof (*obj->hdr));
...
static bool rz_bin_mdmp_init_directory_entry(struct rz_bin_mdmp_obj *obj, struct minidump_directory *entry) {
	...
	** this! But we can ensure that the data is not outside of the file */
	...
	case MODULE_LIST_STREAM:
		r = rz_buf_read_at (obj->b, entry->location.rva, (ut8 *)&module_list, sizeof (module_list));
	...
	case MEMORY_64_LIST_STREAM:
		r = rz_buf_read_at (obj->b, entry->location.rva, (ut8 *)&memory64_list, sizeof (memory64_list));
...
static bool rz_bin_mdmp_init_directory(struct rz_bin_mdmp_obj *obj) {
	...
	for (i = 0; i < max_entries; i++) {
		...
		int r = rz_buf_read_at (obj->b, rvadir + delta, (ut8*) &entry, sizeof (struct minidump_directory));
		if (r) {
...
static bool rz_bin_mdmp_patch_pe_headers(RBuffer *pe_buf) {
	...
	rz_buf_read_at (pe_buf, 0, (ut8 *)&dos_hdr, sizeof (Pe64_image_dos_header));
	rz_buf_read_at (pe_buf, dos_hdr.e_lfanew, (ut8 *)&nt_hdr, sizeof (Pe64_image_nt_headers));
	...
	for (i = 0; i < nt_hdr.file_header.NumberOfSections; i++) {
		rz_buf_read_at (pe_buf, sect_hdrs_off + i * sizeof (section_hdr), (ut8 *)&section_hdr, sizeof (section_hdr));
		section_hdr.PointerToRawData = section_hdr.VirtualAddress;
		rz_buf_write_at (pe_buf, sect_hdrs_off + i * sizeof (section_hdr), (const ut8 *)&section_hdr, sizeof (section_hdr));
	}
