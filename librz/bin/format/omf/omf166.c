...
static int load_omf_pedata(const rz_bin_omf166_obj *obj, const ut8 *buf, const OMF_record *record, const ut64 global_ct) {
	...
	if (pe->isVector) {
		...
		rz_pvector_foreach (obj->symbols_vec, it) {
			...
			if (p->is_data)
				continue;
			// printf("p->name2 = `%s` offset: 0x%08x (%d), [%ld], REP8: %02x, ti: 0x%02x\n",
			// 	p->name2, p->base + p->offset, p->rec_type, p->size, p->REP8, p->ti);
			if (p->base + p->offset == pe->offset) {
				printf("p->name2 = `%s` offset: 0x%08x (%d), [%" PFMT64u "], REP8: %02x, ti: 0x%02x\n",
					p->name2, p->base + p->offset, p->rec_type, p->size, p->REP8, p->ti);
			}
		}
	}
...
static int rz_bin_format_omf166_load_all_records(rz_bin_omf166_obj *obj, const ut8 *buf, const ut64 size) {
	...
	RZ_LOG_DEBUG("blocks count: %" PFMT64d "\n", bc);
	if (bc > 0) {
