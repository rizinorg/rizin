...
#define READ_BUF64(x) if (idx+sizeof(ut64)>=len) { return false;} \
	(x)=rz_read_ble64(buf, big_end); idx+=sizeof(ut64);buf+=sizeof(ut64)
#define READ_BUF32(x) if (idx+sizeof(ut32)>=len) { return false;} \
	(x)=rz_read_ble32(buf, big_end); idx+=sizeof(ut32);buf+=sizeof(ut32)
#define READ_BUF16(x) if (idx+sizeof(ut16)>=len) { return false;} \
	(x)=rz_read_ble16(buf, big_end); idx+=sizeof(ut16);buf+=sizeof(ut16)
