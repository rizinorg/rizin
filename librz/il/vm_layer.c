...
RZ_API bool rz_il_vm_init(RzILVM *vm, ut64 start_addr, ut32 addr_size, ut32 data_size) {
	...
	vm->pc = rz_bitvector_new_from_ut64(addr_size, start_addr);

	// Table for storing the core theory opcodes
	HtPPOptions ops_options = { 0 };
	ops_options.cmp = (HtPPListComparator)rz_bitvector_cmp;
	ops_options.hashfn = (HtPPHashFunction)rz_bitvector_hash;
	ops_options.dupkey = (HtPPDupKey)rz_bitvector_dup;
...
RZ_API RzBitVector *rz_il_ut64_addr_to_bv(ut64 addr) {
	return rz_bitvector_new_from_ut64(64, addr);
}
...
RZ_API ut64 rz_bitvector_addr_to_ut64(RzBitVector *addr) {
	return rz_bitvector_to_ut64(addr);
}

/**
 * the same as rz_bitvector_free, free a bitvector address
...
RZ_API RzILMem *rz_il_vm_mem_store_zero(RzILVM *vm, ut32 mem_index, RzBitVector *key, RzBitVector **value) {
	...
	if (vm && vm->mems) {
		...
		RzBitVector *zero = rz_bitvector_new(m->min_unit_size);
...
RZ_API void rz_il_vm_list_step(RzILVM *vm, RzPVector *op_list, ut32 op_size) {
	...
	RzBitVector *step = rz_bitvector_new_from_ut64(vm->pc->len, op_size);
	RzBitVector *next_pc = rz_bitvector_add(vm->pc, step, NULL);
	rz_il_vm_event_add(vm, rz_il_event_pc_write_new(vm->pc, next_pc));
	rz_bitvector_free(vm->pc);
	rz_bitvector_free(step);
	vm->pc = next_pc;
}
