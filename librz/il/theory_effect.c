...
void *rz_il_handler_branch(RzILVM *vm, RzILOp *op, RZIL_OP_ARG_TYPE *type) {
	...
	if (condition->b) {
		true_branch = (op_branch->true_eff == NULL) ? rz_il_effect_new(EFFECT_TYPE_NON) : rz_il_evaluate_effect(vm, op_branch->true_eff, type);
		ret = true_branch;
	} else {
		false_branch = (op_branch->false_eff == NULL) ? rz_il_effect_new(EFFECT_TYPE_NON) : rz_il_evaluate_effect(vm, op_branch->false_eff, type);
		ret = false_branch;
	}
