...
static void map_cpu_memory (RzAnal *anal, int entry, ut32 addr, ut32 size, bool force) {
	...
	if (desc && anal->iob.fd_get_name (anal->iob.io, desc->fd)) {
		...
		if (desc && anal->iob.fd_get_name (anal->iob.io, desc->fd)) {
			RzList *maps = anal->iob.fd_get_map (anal->iob.io, desc->fd);
			RzIOMap *current_map;
			RzListIter *iter;
...
static int i8051_hook_reg_read(RzAnalEsil *esil, const char *name, ut64 *res, int *size) {
	...
	if ((ri = i8051_reg_find (name))) {
		...
		ret = rz_anal_esil_mem_read (esil, IRAM_BASE + offset, (ut8*)res, ri->num_bytes);
	}
	esil->cb = ocbs;
...
//		if 0xffffffnn, addr = ([SFRnn] << 8) + _xdata (TODO)
	...
	int retval = rz_reg_set_profile_string (anal->reg, p);
	if (retval) {
