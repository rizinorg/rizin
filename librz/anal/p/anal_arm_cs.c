...
static RzRegItem regdelta_regs[4];
	...
	case ARM64_INS_LDARH:
		...
		if (ISMEM64(1)) {
			if (HASMEMINDEX64(1)) {
				...
				if (LSHIFT2_64(1)) {
					rz_strbuf_appendf (&op->esil, "%s,%d,%"PFMT64d",%s,+,DUP,tmp,=,[%d],%s,=",
	...
	case ARM64_INS_LDURSW:
		...
		if (ISMEM64(1)) {
			if (HASMEMINDEX64(1)) {
				...
				if (LSHIFT2_64(1)) {
					rz_strbuf_appendf (&op->esil, "%d,%s,%d,%"PFMT64d",%s,+,DUP,tmp,=,[%d],~,%s,=",
	...
	case ARM64_INS_STR: // str x6, [x6,0xf90]
		...
		if (ISMEM64(1)) {
			if (HASMEMINDEX64(1)) {
				...
				if (LSHIFT2_64(1)) {
					rz_strbuf_appendf (&op->esil, "%s,%s,%d,%"PFMT64d",%s,+,DUP,tmp,=,=[%d]",
...
// 0x000082a8    28301be5     ldr r3, [fp, -0x28]
	...
	case ARM_INS_BLX:
		...
		if (ISREG(0)) {
			...
			// rz_anal_hint_set_bits (a, op->jump, a->bits == 32? 16 : 32);
		}
		break;
	case ARM_INS_BL:
		/* bl label */
...
static int parse_reg_name(RzRegItem *reg_base, RzRegItem *reg_delta, csh handle, cs_insn *insn, int reg_num) {
...
static int parse_reg64_name(RzRegItem *reg_base, RzRegItem *reg_delta, csh handle, cs_insn *insn, int reg_num) {
...
static void set_src_dst(RzAnalValue *val, csh *handle, cs_insn *insn, int x, int bits) {
...
static void op_fillval(RzAnalOp *op , csh handle, cs_insn *insn, int bits) {
	create_src_dst (op);
	switch (op->type & RZ_ANAL_OP_TYPE_MASK) {
...
static ut8 *anal_mask(RzAnal *anal, int size, const ut8 *data, ut64 at) {
	...
	while (idx < size) {
		hint = rz_anal_hint_get (anal, at + idx);
		if (hint) {
			if (hint->bits != 0) {
				anal->bits = hint->bits;
			}
