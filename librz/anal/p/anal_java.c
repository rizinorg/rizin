...
static int java_switch_op(RzAnal *anal, RzAnalOp *op, ut64 addr, const ut8 *data, int len) {
	...
	if (op_byte == 0xaa) {
		...
		op->switch_op = rz_anal_switch_op_new (addr, min_val, max_val, default_loc);
		pos += 12;
		if (max_val > min_val && ((max_val - min_val)<(UT16_MAX/4))) {
			//caseop = rz_anal_switch_op_add_case(op->switch_op, addr+default_loc, -1, addr+offset);
...
static int java_op(RzAnal *anal, RzAnalOp *op, ut64 addr, const ut8 *data, int len, RzAnalOpMask mask) {
	...
	*/
	...
	op->eob = rz_anal_java_is_op_type_eop (op->type2);
	IFDBG {
		const char *ot_str = rz_anal_optype_to_string (op->type);
