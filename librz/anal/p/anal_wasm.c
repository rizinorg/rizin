		...
		case WASM_OP_END:
			{
				...
				if (scope_hint < UT64_MAX) {
					...
					if (hint && !strncmp ("loop", hint->opcode, 4)) {
						rz_anal_hint_set_jump (anal, addr, op->jump);
						rz_anal_hint_set_jump (anal, op->jump, addr);
					} else if (hint && !strncmp ("block", hint->opcode, 5)) {
						// if/else/block
						rz_anal_hint_set_jump (anal, hint->jump, addr);
						rz_anal_hint_set_jump (anal, addr, UT64_MAX);
					}
					if (hint) {
						rz_anal_hint_set_opcode (anal, scope_hint, "invalid");
						rz_anal_hint_set_jump (anal, scope_hint, UT64_MAX);
						rz_anal_hint_del (anal, scope_hint, 1);
						scope_hint++;
					} else {
