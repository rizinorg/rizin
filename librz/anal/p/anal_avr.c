...
INST_HANDLER (eijmp) {	// EIJMP
	...
	rz_anal_esil_reg_read (anal->esil, "z",    &z,    NULL);
	rz_anal_esil_reg_read (anal->esil, "eind", &eind, NULL);
...
INST_HANDLER (spm) {	// SPM Z+
	...
	rz_anal_esil_reg_read (anal->esil, "spmcsr", &spmcsr, NULL);
...
static int avr_op(RzAnal *anal, RzAnalOp *op, ut64 addr, const ut8 *buf, int len, RzAnalOpMask mask) {
	...
	if (anal->esil) {
		offset = 0;
		rz_anal_esil_reg_write (anal->esil, "_prog", offset);

		offset += (1 << cpu->pc);
		rz_anal_esil_reg_write (anal->esil, "_io", offset);

		offset += const_get_value (const_by_name (cpu, CPU_CONST_PARAM, "sram_start"));
		rz_anal_esil_reg_write (anal->esil, "_sram", offset);

		offset += const_get_value (const_by_name (cpu, CPU_CONST_PARAM, "sram_size"));
		rz_anal_esil_reg_write (anal->esil, "_eeprom", offset);

		offset += const_get_value (const_by_name (cpu, CPU_CONST_PARAM, "eeprom_size"));
		rz_anal_esil_reg_write (anal->esil, "_page", offset);
	}
	// process opcode
...
static bool avr_custom_des (RzAnalEsil *esil) {
	...
	rz_anal_esil_reg_read (esil, "hf", &encrypt, NULL);
	rz_anal_esil_reg_read (esil, "deskey", &key, NULL);
	rz_anal_esil_reg_read (esil, "text", &text, NULL);
	...
	rz_anal_esil_reg_write (esil, "text", text);
	return true;
}
...
static bool avr_custom_spm_page_fill(RzAnalEsil *esil) {
	...
	rz_anal_esil_mem_write (esil, addr++, &r0, 1);
	rz_anal_esil_mem_write (esil, addr++, &r1, 1);
...
static bool avr_custom_spm_page_write(RzAnalEsil *esil) {
	...
	cpu = get_cpu_model (esil->anal->cpu);
	page_size_bits = const_get_value (const_by_name (cpu, CPU_CONST_PARAM, "page_size"));
	rz_anal_esil_reg_read (esil, "_page", &tmp_page, NULL);
	...
	rz_anal_esil_mem_read (esil, tmp_page, (ut8 *) t, 1 << page_size_bits);
	rz_anal_esil_mem_write (esil, addr, (ut8 *) t, 1 << page_size_bits);
