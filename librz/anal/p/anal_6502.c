...
static void _6502_anal_esil_ccall(RzAnalOp *op, ut8 data0) {
	...
	rz_strbuf_setf (&op->esil, "%s,?{,0x%04x,pc,=,}", flag, (op->jump & 0xffff));
}
...
static int _6502_op(RzAnal *anal, RzAnalOp *op, ut64 addr, const ut8 *data, int len, RzAnalOpMask mask) {
	...
	case 0x20: // jsr $ffff
		...
		rz_strbuf_setf (&op->esil, "1,pc,-,0xff,sp,+,=[2],0x%04x,pc,=,2,sp,-=", op->jump);
		break;
	// JMP
	case 0x4c: // jmp $ffff
		op->cycles = 3;
		...
		op->jump = (len > 2)? data[1] | data[2] << 8: 0;
		rz_strbuf_setf (&op->esil, "0x%04x,pc,=", op->jump);
		break;
	case 0x6c: // jmp ($ffff)
		op->cycles = 5;
...
static int esil_6502_init (RzAnalEsil *esil) {
	if (esil->anal && esil->anal->reg) {		//initial values
		...
		rz_reg_set_value (esil->anal->reg, rz_reg_get (esil->anal->reg, "a", -1), 0x00);
		rz_reg_set_value (esil->anal->reg, rz_reg_get (esil->anal->reg, "x", -1), 0x00);
		rz_reg_set_value (esil->anal->reg, rz_reg_get (esil->anal->reg, "y", -1), 0x00);
		rz_reg_set_value (esil->anal->reg, rz_reg_get (esil->anal->reg, "flags", -1), 0x00);
	}
	return true;
}
