...
static int riscv_op(RzAnal *anal, RzAnalOp *op, ut64 addr, const ut8 *data, int len, RzAnalOpMask mask) {
	...
	if (len >= sizeof (ut64)) {
		word = rz_read_ble64 (data, anal->big_endian);
	} else if (len >= sizeof (ut32)) {
		word = rz_read_ble16 (data, anal->big_endian);
	} else {
...
// memory
	...
	if (mask & R_ANAL_OP_MASK_VAL && args.num) {
		...
		for (i = 0; j < args.num; i++, j++) {
			op->src[i] = R_NEW0 (RzAnalValue);
			comma = strtok (NULL, ",");
			if (comma && strchr (comma, '(')) {
				op->src[i]->delta = (st64)rz_num_get (NULL, args.arg[j]);
				op->src[i]->reg = rz_reg_get (anal->reg, args.arg[j + 1], -1);
				j++;
			} else if (isalpha (args.arg[j][0])) {
				op->src[i]->reg = rz_reg_get (anal->reg, args.arg[j], -1);
			} else {
