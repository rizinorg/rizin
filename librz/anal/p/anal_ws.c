...
static int ws_anal(RzAnal *anal, RzAnalOp *op, ut64 addr, const ut8 *data, int len, RzAnalOpMask mask) {
	...
	if (op->size) {
		...
		case 'e':
			...
			break;
		case 'd':
			op->type = (buf_asm[1] == 'u')? RZ_ANAL_OP_TYPE_UPUSH: RZ_ANAL_OP_TYPE_DIV;
			break;
		case 'i':
		...
		case 'a':
			...
			break;
		case 'm':
			op->type = (buf_asm[1] == 'o') ? RZ_ANAL_OP_TYPE_MOD : RZ_ANAL_OP_TYPE_MUL;
			break;
		case 'r':
			...
			if (buf_asm[1] == 'o') {
				...
				if (buf_asm[2] == 's') {
					...
					if (127 > atoi (buf_asm + 5) && atoi (buf_asm + 5) >= 33) {
						...
						rz_meta_set_string (anal, RZ_META_TYPE_COMMENT, addr, c);
					}
				} else {
					op->type = RZ_ANAL_OP_TYPE_IO;
				}
		...
		case 's':
			...
			case 't':
				...
				break;
			case 'l':
				op->type = RZ_ANAL_OP_TYPE_LOAD;	// XXX
				break;
			case 'w':
