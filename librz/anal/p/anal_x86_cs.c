...
static void opex(RStrBuf *buf, cs_insn *insn, int mode) {
	...
	rz_strbuf_appendf (buf, "\"operands\":[", x->op_count);
	for (i = 0; i < x->op_count; i++) {
		...
		case X86_OP_IMM:
			...
			rz_strbuf_appendf (buf, ",\"value\":%"PFMT64u, op->imm);
			break;
		case X86_OP_MEM:
			...
			rz_strbuf_appendf (buf, ",\"disp\":%"PFMT64u"", op->mem.disp);
			break;
		default:
	...
	if (x->disp) {
		rz_strbuf_appendf (buf, ",\"disp\":%d", x->disp);
	}
	if (x->sib_index) {
...
static void anop_esil(RzAnal *a, RzAnalOp *op, ut64 addr, const ut8 *buf, int len, csh *handle, cs_insn *insn) {
	...
	case X86_INS_LJMP:
		...
		case X86_OP_REG:
			...
			op->src[0]->reg = rz_reg_get (a->reg, src, R_REG_TYPE_GPR);
			//XXX fallthrough
			}
		//case X86_OP_FP:
		default: // other?
...
static void set_access_info(RzReg *reg, RzAnalOp *op, csh *handle, cs_insn *insn, int mode) {
	...
	RzList *ret = rz_list_newf ((RzListFree)rz_anal_value_free);
	if (!ret) {
		return;
	}

	// PC register
	val = rz_anal_value_new ();
...
static int x86_int_0x80(RzAnalEsil *esil, int interrupt) {
	...
	if (!esil || (interrupt != 0x80))
		return false;
	rz_anal_esil_reg_read (esil, "eax", &eax, NULL);
	rz_anal_esil_reg_read (esil, "ebx", &ebx, NULL);
	rz_anal_esil_reg_read (esil, "ecx", &ecx, NULL);
	rz_anal_esil_reg_read (esil, "edx", &edx, NULL);
	syscall = (int) eax;
	switch (syscall) {
	case 3:
		{
			...
			rz_anal_esil_mem_write (esil, ecx, (ut8 *)dst, (int)edx);
			free (dst);
			return true;
		}
	case 4:
		{
			...
			rz_anal_esil_mem_read (esil, ecx, (ut8 *)src, (int)edx);
...
static int esil_x86_cs_init(RzAnalEsil *esil) {
	...
	// rz_anal_esil_set_interrupt (esil, 0x80, x86_int_0x80);
	/* disable by default */
//	rz_anal_esil_set_interrupt (esil, 0x80, NULL);	// this is stupid, don't do this
	return true;
}
...
static char *get_reg_profile(RzAnal *anal) {
	...
	case 64:
	{
		const char *cc = rz_anal_cc_default (anal);
