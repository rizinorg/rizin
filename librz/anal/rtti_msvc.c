...
static void rtti_msvc_print_type_descriptor_json(rtti_type_descriptor *td) {
	rz_cons_printf ("{\"vtable_addr\":%"PFMT32u",\"spare\":%"PFMT32u",\"name\":\"%s\"}",
...
static char *unique_class_name(RzAnal *anal, const char *original_name) {
	...
	} while (rz_anal_class_exists (anal, name));
...
static void recovery_apply_vtable(RzAnal *anal, const char *class_name, RVTableInfo *vtable_info) {
	...
	rz_anal_class_vtable_set (anal, class_name, &vtable);
	...
	rz_vector_foreach (&vtable_info->methods, vmeth) {
		...
		rz_anal_class_method_set (anal, class_name, &meth);
...
static void recovery_apply_bases(RRTTIMSVCAnalContext *context, const char *class_name, RzVector *base_descs) {
	...
	rz_vector_foreach (base_descs, base_desc) {
		...
		rz_anal_class_base_set (context->vt_context->anal, class_name, &base);
...
static const char *recovery_apply_complete_object_locator(RRTTIMSVCAnalContext *context, RecoveryCompleteObjectLocator *col) {
	...
	char *name = rz_anal_rtti_msvc_demangle_class_name (context->vt_context, col->td->td.name);
	if (!name) {
		if (context->vt_context->anal->verbose) {
	...
	rz_anal_class_create (anal, name);
...
static const char *recovery_apply_type_descriptor(RRTTIMSVCAnalContext *context, RecoveryTypeDescriptor *td) {
	...
	char *name = rz_anal_rtti_msvc_demangle_class_name (context->vt_context, td->td.name);
	if (!name) {
		if (context->vt_context->anal->verbose) {
			eprintf("Failed to demangle a class name: \"%s\"\n", td->td.name);
		}
