...
static bool rtti_itanium_read_type_name(RVTableContext *context, ut64 addr, class_type_info *cti) {
	...
	cti->name = rz_anal_rtti_itanium_demangle_class_name (context, (char *)buf);
	if (!cti->name) {
		return false;
	}
	return true;
...
static char *rtti_itanium_read_type_name_custom(RVTableContext *context, ut64 addr, ut64 *str_addr, bool *unique_name) {
	...
	*str_addr = at;
	...
	buf[NAME_BUF_SIZE - 1] = 0;
	char *name = rz_anal_rtti_itanium_demangle_class_name (context, (char *)buf);
...
RZ_API void rz_anal_rtti_itanium_recover_all(RVTableContext *context, RzList *vtables) {
	...
	rz_list_foreach (vtables, iter, vtable) {
		...
		rz_anal_class_create (context->anal, cti->name);
