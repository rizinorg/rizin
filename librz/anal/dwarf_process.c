		...
		for (j = idx; child_depth > 0 && j < ctx->count; j++) {
			...
			if (child_depth == 1 && child_die->tag == DW_TAG_subrange_type) {
				...
				for (i = 0; i < child_die->count; i++) {
					...
					case DW_AT_count:
						rz_strbuf_appendf (strbuf, "[%d]", value->uconstant + 1);
						break;
...
static void parse_structure_type(Context *ctx, ut64 idx) {
	...
	rz_anal_save_base_type (ctx->anal, base_type);
cleanup:
...
static RBinDwarfLocRange *find_largest_loc_range (RzList *loc_list) {
...
RZ_API void rz_anal_dwarf_integrate_functions(RzAnal *anal, RzFlag *flags, Sdb *dwarf_sdb) {
	...
	ls_foreach (sdb_list, it, kv) {
		...
		if (fcn) {
			...
			rz_anal_function_rename (fcn, dwf_name);
			free (dwf_name);
		...
		sdb_aforeach (var_name, vars) {
			...
			if (*kind == 'g') { /* global, fixed addr TODO add size to variables? */
				...
				rz_anal_function_set_var (fcn, offset - fcn->maxstack, *kind, type, 4, false, var_name);
			} else if (*kind == 'r' && fcn) {
				RzRegItem *i = rz_reg_get (anal->reg, extra, -1);
				if (!i) {
					goto loop_end;
				}
				rz_anal_function_set_var (fcn, i->index, *kind, type, 4, false, var_name);
			} else if (fcn) { /* kind == 'b' */
				rz_anal_function_set_var (fcn, offset - fcn->bp_off, *kind, type, 4, false, var_name);
			}
			free (var_key);
			free (var_data);
		loop_end:
