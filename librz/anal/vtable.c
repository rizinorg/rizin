...
static bool vtable_is_addr_vtable_start_msvc(RVTableContext *context, ut64 curAddress) {
	...
	RzList *xrefs = rz_anal_xrefs_get (context->anal, curAddress);
	...
	rz_list_foreach (xrefs, xrefIter, xref) {
		...
		if (vtable_addr_in_text_section (context, xref->addr)) {
			...
			RzAnalOp analop = { 0 };
			rz_anal_op (context->anal, &analop, xref->addr, buf, sizeof(buf), R_ANAL_OP_MASK_BASIC);
...
RZ_API RVTableInfo *rz_anal_vtable_parse_at(RVTableContext *context, ut64 addr) {
	...
	while (vtable_is_value_in_text_section (context, addr, &meth.addr)) {
		...
		RzList *ll = rz_anal_xrefs_get (context->anal, addr);
		if (!rz_list_empty (ll)) {
			rz_list_free (ll);
			break;
		}
...
RZ_API RzList *rz_anal_vtable_search(RVTableContext *context) {
	...
	RzList *vtables = rz_list_newf ((RzListFree)rz_anal_vtable_info_free);
	if (!vtables) {
		return NULL;
	}
	...
	rz_list_foreach (sections, iter, section) {
		...
		while (startAddress <= endAddress) {
			...
			if (vtable_is_addr_vtable_start (context, section, startAddress)) {
				RVTableInfo *vtable = rz_anal_vtable_parse_at (context, startAddress);
				if (vtable) {
					rz_list_append (vtables, vtable);
					ut64 size = rz_anal_vtable_info_get_size (context, vtable);
					if (size > 0) {
						startAddress += size;
						continue;
					}
	...
	if (rad == 'j') {
		...
		rz_list_foreach (vtables, vtableIter, table) {
			rz_cons_printf ("f vtable.0x%08"PFMT64x" %"PFMT64d" @ 0x%08"PFMT64x"\n",
						   table->saddr,
						   rz_anal_vtable_info_get_size (&context, table),
						   table->saddr);
			rz_vector_foreach (&table->methods, curMethod) {
