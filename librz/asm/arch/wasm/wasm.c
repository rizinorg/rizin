	...
	if (id < 0xc0) {
		...
		case WASM_OP_IF: {
			...
			case RZ_BIN_WASM_VALUETYPE_v128:
				...
				break;
			default:
				rz_strbuf_setf(sb, "%s (result ?)", opdef->txt);
				break;
			}
			op->len += n;
		} break;
		...
		case WASM_OP_CALL: {
			...
			rz_strbuf_setf(sb, "%s %d", opdef->txt, val);
			op->len += n;
		} break;
		case WASM_OP_BRTABLE: {
			...
			op->len += n;
			rz_strbuf_setf(sb, "%s %d ", opdef->txt, count);
			for (i = 0; i < count; i++) {
				rz_strbuf_appendf(sb, "%d ", table[i]);
			}
			rz_strbuf_appendf(sb, "%d", def);
			free(table);
			break;
		beach:
			free(table);
		...
		case WASM_OP_CALLINDIRECT: {
			...
			reserved &= 0x1;
			rz_strbuf_setf(sb, "%s %d %d", opdef->txt, val, reserved);
			op->len += n;
		} break;
		...
		case WASM_OP_I64STORE32: {
			...
			rz_strbuf_setf(sb, "%s %d %d", opdef->txt, flag, offset);
			op->len += n;
		} break;
		...
		case WASM_OP_GROWMEMORY: {
			...
			reserved &= 0x1;
			rz_strbuf_setf(sb, "%s %d", opdef->txt, reserved);
			op->len += n;
		} break;
		case WASM_OP_I32CONST: {
			...
			rz_strbuf_setf(sb, "%s %" PFMT32d, opdef->txt, val);
			op->len += n;
		} break;
		case WASM_OP_I64CONST: {
			...
			rz_strbuf_setf(sb, "%s %" PFMT64d, opdef->txt, val);
			op->len += n;
		} break;
		case WASM_OP_F32CONST:
			if (buf_len > 4) {
				...
				u.v = rz_read_at_le32(buf, 1);
				rz_strbuf_setf(sb, "%s %f", opdef->txt, u.f);
				op->len += 4;
			} else {
				goto err;
			}
		...
		case WASM_OP_V128STORE: {
			...
			rz_strbuf_setf(sb, "%s o:%d a:2^%d", opdef->txt, offset, align);
		} break;
		...
		case WASM_OP_V8X16SHUFFLE: {
			...
			rz_strbuf_setf(sb, "%s %02x %02x %02x %02x %02x %02x %02x "
		...
		case WASM_OP_F64X2REPLACELANE: {
			...
			++op->len;
			rz_strbuf_setf(sb, "%s %d", opdef->txt, lane);
		} break;
		}
	} else {
		goto err;
