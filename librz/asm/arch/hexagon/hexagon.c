...
static inline bool is_single_instr_pkt(const HexPktInfo* i_pkt_info) {
    return (i_pkt_info->first_insn && i_pkt_info->last_insn);
}
...
void hex_set_pkt_info(HexPktInfo* i_pkt_info) {
    ...
    if (new_pkt_starts && is_last_instr(i_pkt_info->parse_bits)) {  // Single instruction packet.
        ...
        strncpy(i_pkt_info->syntax_prefix, "[  ", 4);
        ...
        strncpy(i_pkt_info->syntax_prefix, "┌", 4);  // TODO Add non utf8 option "/"
        i_pkt_info->first_insn = true;
        ...
        if (is_endloop01_pkt(p0, p1)) {
            strncpy(i_pkt_info->syntax_postfix, " ∎ endloop01", 16);
        ...
        strncpy(i_pkt_info->syntax_prefix, "│", 4);  // TODO AAdd non utf8 option "|"
        new_pkt_starts = false;
        i = (i + 1) % 4;
    }
