...
int lua53_disasm(RzAsmOp *op, const ut8 *buf, int len, LuaOpNameList opnames) {
	...
	char *asm_string;
		...
		asm_string = luaop_new_str_2arg_ex(opnames[opcode], a, b, LUA_NO_PREFIX, LUA_NO_PREFIX, NULL);
		break;
        case OP_GETUPVAL:	/*  A B     R(A) := UpValue[B]                              */
		asm_string = luaop_new_str_2arg_ex(opnames[opcode], a, b, LUA_REG_PREF, LUA_UPVALUE_PREF, NULL);
		break;
        case OP_TEST:		/*      A C     if not (R(A) <=> C) then pc++                   */
		asm_string = luaop_new_str_2arg_ex(opnames[opcode], a, c, LUA_REG_PREF, LUA_NO_PREFIX, NULL);
		break;
        case OP_TFORCALL:	/*  A C     R(A+3), ... ,R(A+2+C) := R(A)(R(A+1), R(A+2));  */
		asm_string = luaop_new_str_2arg_ex(opnames[opcode], a, c, LUA_NO_PREFIX, LUA_NO_PREFIX, NULL);
                break;
        case OP_LOADK:		/*     A Bx    R(A) := Kst(Bx)                                 */
		asm_string = luaop_new_str_2arg_ex(opnames[opcode], a, bx, LUA_REG_PREF, LUA_CONST_PREF, NULL);
                break;
        case OP_CLOSURE:	/*   A Bx    R(A) := closure(KPROTO[Bx])                     */
		asm_string = luaop_new_str_2arg_ex(opnames[opcode], a, bx, LUA_REG_PREF, LUA_KPROTO_PREF, LUA_CLOSURE_MARK);
                break;
