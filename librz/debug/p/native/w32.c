...
int w32_first_thread(int pid) {
	...
	do {
		...
		if (te32.th32OwnerProcessID == pid) {
			...
			if (!thid) {
				rz_sys_perror ("w32_first_thread/OpenThread");
				goto err_load_th;
			}
			CloseHandle (th);
...
err_get_file_name_from_handle:
	...
	if (filename) {
		char *filename_ = rz_sys_conv_win_to_utf8(filename);
		free (filename);
		return filename_;
...
/*
static char * rz_debug_get_dll(void) {
	return lstLibPtr->Path;
}
*/
static  PLIB_ITEM  rz_debug_get_lib_item(void) {
	return lstLibPtr;
}
#define PLIB_MAX 512
static void rz_debug_lstLibAdd(DWORD pid,LPVOID lpBaseOfDll, HANDLE hFile,char * dllname) {
	int x;
	if (lstLib == 0)
...
static void * rz_debug_findlib (void * BaseOfDll) {
...
static  PTHREAD_ITEM  rz_debug_get_thread_item (void) {
	return lstThreadPtr;
}
#define PTHREAD_MAX 1024
static void rz_debug_lstThreadAdd (DWORD pid, DWORD tid, HANDLE hThread, LPVOID  lpThreadLocalBase, LPVOID lpStartAddress, BOOL bFinished) {
	int x;
...
static void * rz_debug_findthread (int pid, int tid) {
...
int w32_dbg_wait(RzDebug *dbg, int pid) {
	...
	do {
		...
		case EXIT_PROCESS_DEBUG_EVENT:
			...
			rz_cons_printf ("(%d) Process %d exited with exit code %d\n", (int)de.dwProcessId, (int)de.dwProcessId,
				(int)de.u.ExitProcess.dwExitCode);
			rz_cons_flush ();
			//debug_load();
			next_event = 0;
		...
		case EXIT_THREAD_DEBUG_EVENT:
			...
			lstThreadPtr = (PTHREAD_ITEM)rz_debug_findthread (pid, tid);
			if (lstThreadPtr) {
				lstThreadPtr->bFinished = TRUE;
				lstThreadPtr->dwExitCode = de.u.ExitThread.dwExitCode;
			} else {
				rz_debug_lstThreadAdd (pid, tid, de.u.CreateThread.hThread, de.u.CreateThread.lpThreadLocalBase, de.u.CreateThread.lpStartAddress, TRUE);
			}
			//rz_debug_native_continue (dbg, pid, tid, -1);
			next_event = 0;
		...
		case LOAD_DLL_DEBUG_EVENT:
			...
			rz_debug_lstLibAdd (pid,de.u.LoadDll.lpBaseOfDll, de.u.LoadDll.hFile, dllname);
			if (dllname) {
				free (dllname);
			}
			next_event = 0;
		...
		case UNLOAD_DLL_DEBUG_EVENT:
			...
			lstLibPtr = (PLIB_ITEM)rz_debug_findlib (de.u.UnloadDll.lpBaseOfDll);
			if (lstLibPtr != NULL) {
				lstLibPtr->hFile = (HANDLE)-1;
			} else {
				rz_debug_lstLibAdd (pid, de.u.UnloadDll.lpBaseOfDll, (HANDLE)-1, "not cached");
				if (dllname)
					free (dllname);
			}
			next_event = 0;
...
static HANDLE w32_open_thread(int pid, int tid) {
	...
	if (thread == INVALID_HANDLE_VALUE) {
		rz_sys_perror ("w32_open_thread/OpenThread");
	}
	return thread;
}
...
RzList *w32_thread_list(int pid, RzList *list) {
        ...
        do {
            ...
            if (te32.th32OwnerProcessID == pid) {
				...
				thid = w32_OpenThread (THREAD_ALL_ACCESS, 0, te32.th32ThreadID);
				if (!thid) {
					rz_sys_perror ("w32_thread_list/OpenThread");
					goto err_load_th;
				}
				rz_list_append (list, rz_debug_pid_new ("???", te32.th32ThreadID, 0, 's', 0));
            }
        } while (Thread32Next (th, &te32));
err_load_th:
        if(th != INVALID_HANDLE_VALUE)
...
static RzDebugPid *build_debug_pid(PROCESSENTRY32 *pe) {
	...
	*image_name = '\0';
	...
	if (*image_name) {
		name = rz_sys_conv_win_to_utf8 (image_name);
	} else {
		name = rz_sys_conv_win_to_utf8 (pe->szExeFile);
	}
	ret = rz_debug_pid_new (name, pe->th32ProcessID, 0, 's', 0);
	free (name);
	return ret;
}
...
RzList *w32_pids(int pid, RzList *list) {
	...
	if (process_snapshot == INVALID_HANDLE_VALUE) {
		rz_sys_perror ("w32_pids/CreateToolhelp32Snapshot");
		return list;
	}
	if (!Process32First (process_snapshot, &pe)) {
		rz_sys_perror ("w32_pids/Process32First");
	...
	do {
		if (show_all_pids ||
			...
			RzDebugPid *debug_pid = build_debug_pid (&pe);
			if (debug_pid) {
				rz_list_append (list, debug_pid);
			}
		}
...
bool w32_terminate_process(RzDebug *dbg, int pid) {
	...
	bool ret = false;
	if (!h_proc) {
		rz_sys_perror ("w32_terminate_process/OpenProcess");
	...
	if (ret_wait == WAIT_FAILED) {
		rz_sys_perror ("w32_terminate_process/WaitForSingleObject");
		goto err_w32_terminate_process;
	}
...
void w32_break_process(void *d) {
	RzDebug *dbg = (RzDebug *)d;
	HANDLE h_proc = w32_OpenProcess (PROCESS_ALL_ACCESS, FALSE, dbg->pid);
	if (!h_proc) {
		rz_sys_perror ("w32_break_process/w32_OpenProcess");
		goto err_w32_break_process;
	}
	if (!w32_DebugBreakProcess (h_proc)) {
		rz_sys_perror ("w32_break_process/w32_DebugBreakProcess");
		goto err_w32_break_process;
	}
...
static int w32_reg_read(RzDebug *dbg, int type, ut8 *buf, int size) {
#ifdef _MSC_VER
	CONTEXT ctx;
#else
	...
	if (GetThreadContext (hThread, &ctx) == TRUE) {
		...
		rz_sys_perror ("w32_reg_read/GetThreadContext");
		size = 0;
	}
	if (showfpu) {
...
static int w32_reg_write(RzDebug *dbg, int type, const ut8* buf, int size) {
	BOOL ret = false;
	HANDLE thread;
#if _MSC_VER
	CONTEXT ctx;
...
static void w32_info_user(RzDebug *dbg, RzDebugInfo *rdi) {
	...
	*usr_dom = '\0';
	if (!LookupAccountSid (NULL, tok_usr->User.Sid, usr, &usr_len, usr_dom, &usr_dom_len, &snu)) {
		rz_sys_perror ("w32_info_user/LookupAccountSid");
		goto err_w32_info_user;
	}
	if (*usr_dom) {
...
static void w32_info_exe(RzDebug *dbg, RzDebugInfo *rdi) {
	...
	if (!h_proc) {
		rz_sys_perror ("w32_info_exe/OpenProcess");
	...
	if (w32_QueryFullProcessImageName (h_proc, 0, path, &len)) {
		path[len] = '\0';
		rdi->exe = rz_sys_conv_win_to_utf8 (path);
	} else {
		rz_sys_perror ("w32_info_exe/QueryFullProcessImageName");
	}
err_w32_info_exe:
	if (h_proc) {
		CloseHandle (h_proc);
...
RzDebugInfo *w32_info(RzDebug *dbg, const char *arg) {
	...
	rdi->thread = (void *)rz_debug_get_thread_item ();
	rdi->uid = -1;
	rdi->gid = -1;
	rdi->cwd = NULL;
	rdi->exe = NULL;
