...
# define KINFO_PROC kinfo_proc
	...
	if (pid) {
		...
		if (cnt == 1) {
			RzDebugPid *p = rz_debug_pid_new (KP_COMM (kp), pid, KP_UID (kp), 's', 0);
			if (p) rz_list_append (list, p);
			/* we got our process, now fetch the parent process */
			kp = KVM_GETPROCS (kd, KERN_PROC_PID, KP_PPID(kp), &cnt);
                        if (cnt == 1) {
				RzDebugPid *p = rz_debug_pid_new (KP_COMM (kp), KP_PID (kp), KP_UID (kp), 's', 0);
				if (p) {
					p->ppid = KP_PPID (kp);
		...
		int i;
		for (i = 0; i < cnt; i++) {
			RzDebugPid *p = rz_debug_pid_new (KP_COMM (kp + i), KP_PID (kp + i), KP_UID (kp), 's', 0);
			if (p) {
				p->ppid = KP_PPID (kp);
				rz_list_append (list, p);
			}
