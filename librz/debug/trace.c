...
RZ_API bool rz_debug_trace_ins_before(RzDebug *dbg) {
	...
	if (!rz_analysis_op (dbg->anal, dbg->cur_op, pc, buf_pc, sizeof (buf_pc), RZ_ANAL_OP_MASK_VAL)) {
...
RZ_API int rz_debug_trace_pc(RzDebug *dbg, ut64 pc) {
	...
	if (rz_analysis_op (dbg->anal, &op, pc, buf, sizeof (buf), RZ_ANAL_OP_MASK_ESIL) < 1) {
...
RZ_API void rz_debug_trace_op(RzDebug *dbg, RzAnalysisOp *op) {
	...
	if (dbg->trace->enabled) {
		if (dbg->anal->esil) {
			rz_analysis_esil_trace_op (dbg->anal->esil, op);
		} else {
			if (dbg->verbose) {
...
static int cmpaddr (const void *_a, const void *_b) {
	...
	return (rz_itv_begin (a->pitv) > rz_itv_begin (b->pitv))? 1:
		 (rz_itv_begin (a->pitv) < rz_itv_begin (b->pitv))? -1: 0;
}
...
RZ_API RzDebugTracepoint *rz_debug_trace_add (RzDebug *dbg, ut64 addr, int size) {
	...
	rz_analysis_trace_bb (dbg->anal, addr);
