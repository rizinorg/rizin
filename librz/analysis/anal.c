...
RZ_API RzAnalysis *rz_analysis_new(void) {
	...
	rz_analysis_hint_storage_init (anal);
	...
	(void)rz_analysis_pin_init (anal);
	(void)rz_analysis_xrefs_init (anal);
	...
	anal->stackptr = 0;
	anal->lineswidth = 0;
	anal->fcns = rz_list_newf (rz_analysis_function_free);
	anal->leaddrs = NULL;
	anal->imports = rz_list_newf (free);
	rz_analysis_set_bits (anal, 32);
	anal->plugins = rz_list_newf ((RzListFree) rz_analysis_plugin_free);
	if (anal->plugins) {
		for (i = 0; anal_static_plugins[i]; i++) {
			rz_analysis_add (anal, anal_static_plugins[i]);
		}
	}
	return anal;
}
...
RZ_API bool rz_analysis_set_reg_profile(RzAnalysis *anal) {
	...
	if (anal && anal->cur && anal->cur->set_reg_profile) {
		...
		char *p = rz_analysis_get_reg_profile (anal);
		if (p && *p) {
...
RZ_API bool rz_analysis_set_triplet(RzAnalysis *anal, const char *os, const char *arch, int bits) {
	...
	free (anal->os);
	anal->os = strdup (os);
	rz_analysis_set_bits (anal, bits);
	return rz_analysis_use (anal, arch);
}
...
RZ_API bool rz_analysis_set_os(RzAnalysis *anal, const char *os) {
	...
	return rz_analysis_set_triplet (anal, os, NULL, -1);
}
...
RZ_API void rz_analysis_set_cpu(RzAnalysis *anal, const char *cpu) {
	...
	int v = rz_analysis_archinfo (anal, RZ_ANAL_ARCHINFO_ALIGN);
	if (v != -1) {
		anal->pcalign = v;
	}
}

RZ_API int rz_analysis_set_big_endian(RzAnalysis *anal, int bigend) {
...
RZ_API ut8 *rz_analysis_mask(RzAnalysis *anal, int size, const ut8 *data, ut64 at) {
	...
	while (idx < size) {
		if ((oplen = rz_analysis_op (anal, op, at, data + idx, size - idx, RZ_ANAL_OP_MASK_BASIC)) < 1) {
			break;
		}
...
RZ_API void rz_analysis_trace_bb(RzAnalysis *anal, ut64 addr) {
	...
	RzListIter *iter2;
	fcni = rz_analysis_get_fcn_in (anal, addr, 0);
	if (fcni) {
...
RZ_API void rz_analysis_colorize_bb(RzAnalysis *anal, ut64 addr, ut32 color) {
	RzAnalysisBlock *bbi;
	bbi = rz_analysis_bb_from_offset (anal, addr);
	if (bbi) {
		bbi->colorize = color;
	}
}
...
RZ_API RzAnalysisOp *rz_analysis_op_hexstr(RzAnalysis *anal, ut64 addr, const char *str) {
	...
	rz_analysis_op (anal, op, addr, buf, len, RZ_ANAL_OP_MASK_BASIC);
	free (buf);
	return op;
}
...
RZ_API void rz_analysis_purge(RzAnalysis *anal) {
	...
	rz_analysis_pin_fini (anal);
	rz_analysis_pin_init (anal);
	sdb_reset (anal->sdb_cc);
	rz_list_free (anal->fcns);
	anal->fcns = rz_list_newf (rz_analysis_function_free);
	rz_analysis_purge_imports (anal);
}
...
RZ_API bool rz_analysis_noreturn_add(RzAnalysis *anal, const char *name, ut64 addr) {
	...
	if (addr != UT64_MAX) {
		if (sdb_bool_set (TDB, K_NORET_ADDR (addr), true, 0)) {
			RzAnalysisFunction *fcn = rz_analysis_get_function_at (anal, addr);
			if (fcn) {
	...
	if (name && *name) {
		...
		RzAnalysisFunction *fcn = rz_analysis_get_fcn_in (anal, addr, -1);
...
RZ_API bool rz_analysis_noreturn_drop(RzAnalysis *anal, const char *expr) {
	...
	if (!strncmp (expr, "0x", 2)) {
		...
		RzAnalysisFunction *fcn = rz_analysis_get_fcn_in (anal, n, -1);
		if (!fcn) {
...
static bool rz_analysis_noreturn_at_name(RzAnalysis *anal, const char *name) {
	...
	if (rz_str_startswith (name, "reloc.")) {
		return rz_analysis_noreturn_at_name (anal, name + 6);
	}
	return false;
}
...
static bool noreturn_recurse(RzAnalysis *anal, ut64 addr) {
	...
	if (rz_analysis_op (anal, &op, addr, bbuf, sizeof (bbuf), RZ_ANAL_OP_MASK_BASIC | RZ_ANAL_OP_MASK_VAL) < 1) {
		return false;
	}
	...
	return rz_analysis_noreturn_at (anal, recurse_addr);
}
...
RZ_API bool rz_analysis_noreturn_at(RzAnalysis *anal, ut64 addr) {
	...
	if (rz_analysis_noreturn_at_addr (anal, addr)) {
		return true;
	}
	/* XXX this is very slow */
	RzAnalysisFunction *f = rz_analysis_get_function_at (anal, addr);
	if (f) {
		if (rz_analysis_noreturn_at_name (anal, f->name)) {
			return true;
		}
	}
	RzFlagItem *fi = anal->flag_get (anal->flb.f, addr);
	if (fi) {
		if (rz_analysis_noreturn_at_name (anal, fi->realname ? fi->realname : fi->name)) {
			return true;
		}
	}
...
RZ_API void rz_analysis_bind(RzAnalysis *anal, RzAnalysisBind *b) {
	if (b) {
		b->anal = anal;
...
RZ_API bool rz_analysis_is_prelude(RzAnalysis *anal, const ut8 *data, int len) {
	RzList *l = rz_analysis_preludes (anal);
	if (l) {
		RzSearchKeyword *kw;
		RzListIter *iter;
