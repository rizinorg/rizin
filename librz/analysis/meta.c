	...
	if (str || d->type == RZ_META_TYPE_DATA) {
		...
		default:
			...
			case RZ_META_TYPE_STRING:
				if (mode == RZ_OUTPUT_MODE_RIZIN || mode == RZ_OUTPUT_MODE_SDB || mode == RZ_OUTPUT_MODE_TABLE || mode == RZ_OUTPUT_MODE_LONG || mode == RZ_OUTPUT_MODE_LONG_JSON || mode == RZ_OUTPUT_MODE_QUIET) {
					char cmd[] = "Cs#";
					switch (d->subtype) {
					case 'a':
					case '8':
