...
static void shadow_var_struct_members(RzAnalysisVar *var) {
	Sdb *TDB = var->fcn->analysis->type->sdb_types;
...
static void extract_arg(RzAnalysis *analysis, RzAnalysisFunction *fcn, RzAnalysisOp *op, const char *reg, const char *sign, char type) {
	...
	if (*sign == '+') {
		...
		if (isarg) {
			...
			char *fname = rz_type_func_guess(analysis->type, fcn->name);
			if (fname) {
				ut64 sum_sz = 0;
				size_t from, to, i;
				if (stack_rev) {
					const size_t cnt = rz_type_func_args_count(analysis->type, fname);
					from = cnt ? cnt - 1 : cnt;
					to = fcn->cc ? rz_analysis_cc_max_arg(analysis, fcn->cc) : 0;
				} else {
					from = fcn->cc ? rz_analysis_cc_max_arg(analysis, fcn->cc) : 0;
					to = rz_type_func_args_count(analysis->type, fname);
				}
				const int bytes = (fcn->bits ? fcn->bits : analysis->bits) / 8;
				for (i = from; stack_rev ? i >= to : i < to; stack_rev ? i-- : i++) {
					char *tp = rz_type_func_args_type(analysis->type, fname, i);
					if (!tp) {
						break;
					}
					if (sum_sz == frame_off) {
						vartype = tp;
						varname = strdup(rz_type_func_args_name(analysis->type, fname, i));
						break;
					}
					ut64 bit_sz = rz_type_get_bitsize(analysis->type, tp);
...
RZ_API void rz_analysis_extract_rarg(RzAnalysis *analysis, RzAnalysisOp *op, RzAnalysisFunction *fcn, int *reg_set, int *count) {
	...
	if (fname) {
		argc = rz_type_func_args_count(analysis->type, fname);
	}
	...
	if (is_call && *count < max_count) {
		...
		if (!f) {
			...
			if (flag) {
				callee = rz_type_func_guess(analysis->type, flag->name);
				if (callee) {
					const char *cc = rz_analysis_cc_func(analysis, callee);
					if (cc && !strcmp(fcn->cc, cc)) {
						callee_rargs = RZ_MIN(max_count, rz_type_func_args_count(analysis->type, callee));
					}
				}
			}
		} else if (!f->is_variadic && !strcmp(fcn->cc, f->cc)) {
			callee = rz_type_func_guess(analysis->type, f->name);
			if (callee) {
				callee_rargs = RZ_MIN(max_count, rz_type_func_args_count(analysis->type, callee));
			}
		...
		for (i = 0; i < callee_rargs; i++) {
			...
			if (fname) {
				type = rz_type_func_args_type(analysis->type, fname, i);
				vname = rz_type_func_args_name(analysis->type, fname, i);
			}
			if (!vname && callee) {
				type = rz_type_func_args_type(analysis->type, callee, i);
				vname = rz_type_func_args_name(analysis->type, callee, i);
			}
			if (vname) {
				reg_set[i] = 1;
			} else {
...
RZ_API char *rz_analysis_fcn_format_sig(RZ_NONNULL RzAnalysis *analysis, RZ_NONNULL RzAnalysisFunction *fcn, RZ_NULLABLE char *fcn_name,
	...
	char *type_fcn_name = rz_type_func_guess(analysis->type, fcn_name);
	if (type_fcn_name && rz_type_func_exist(analysis->type, type_fcn_name)) {
		const char *fcn_type = rz_type_func_ret(analysis->type, type_fcn_name);
		if (fcn_type) {
			const char *sp = " ";
	...
	if (type_fcn_name && rz_type_func_exist(analysis->type, type_fcn_name)) {
		int i, argc = rz_type_func_args_count(analysis->type, type_fcn_name);
		bool comma = true;
		// This avoids false positives present in argument recovery
		// and straight away print arguments fetched from types db
		for (i = 0; i < argc; i++) {
			char *type = rz_type_func_args_type(analysis->type, type_fcn_name, i);
			const char *name = rz_type_func_args_name(analysis->type, type_fcn_name, i);
			if (!type || !name) {
...
RZ_API void rz_analysis_fcn_vars_add_types(RzAnalysis *analysis, RzAnalysisFunction *fcn) {
	...
	rz_list_foreach (all_vars, iter, var) {
		if (var->isarg) {
			rz_type_func_arg_set(analysis->type, fcn->name, arg_count, var->name, var->type);
			arg_count++;
		}
	}
	if (arg_count > 0) {
		rz_type_func_arg_count_set(analysis->type, fcn->name, arg_count);
	}
	rz_analysis_fcn_vars_cache_fini(&cache);
}
