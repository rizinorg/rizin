...
static bool block_load_cb(void *user, const char *k, const char *v) {
	...
	RzAnalysisBlock *block = rz_analysis_create_block (ctx->anal, addr, proto.size);
	if (!block) {
		goto error;
	}
...
static bool function_load_cb(void *user, const char *k, const char *v) {
	...
	RzAnalysisFunction *function = rz_analysis_function_new (ctx->anal);
	...
	KEY_PARSER_JSON (ctx->parser, json, child, {
		...
		case FUNCTION_FIELD_BBS: {
			...
			for (baby = child->children.first; baby; baby = baby->next) {
				...
				RzAnalysisBlock *block = rz_analysis_get_block_at (ctx->anal, baby->num.u_value);
				if (!block) {
					continue;
				}
	...
	if (errno || !function->name || !rz_analysis_add_function (ctx->anal, function)) {
...
static bool xrefs_load_cb(void *user, const char *k, const char *v) {
	...
	for (child = json->children.first; child; child = child->next) {
		...
		rz_analysis_xrefs_set (anal, from, to, type);
	}
...
RZ_API void rz_serialize_anal_hints_save(RZ_NONNULL Sdb *db, RZ_NONNULL RzAnalysis *anal) {
	...
	rz_analysis_addr_hints_foreach (anal, addr_hint_acc_cb, acc);
	rz_analysis_arch_hints_foreach (anal, arch_hint_acc_cb, acc);
	rz_analysis_bits_hints_foreach (anal, bits_hint_acc_cb, acc);
...
static bool hints_load_cb(void *user, const char *k, const char *v) {
	...
	KEY_PARSER_JSON (ctx->parser, json, child, {
		case HINTS_FIELD_ARCH:
			rz_analysis_hint_set_arch (anal, addr, child->type == RZ_JSON_STRING ? child->str_value : NULL);
			break;
		case HINTS_FIELD_BITS:
			rz_analysis_hint_set_bits (anal, addr, child->type == RZ_JSON_INTEGER ? (int)child->num.s_value : 0);
			break;
		case HINTS_FIELD_IMMBASE:
			if (child->type != RZ_JSON_INTEGER) {
				break;
			}
			rz_analysis_hint_set_immbase (anal, addr, (int)child->num.s_value);
			break;
		case HINTS_FIELD_JUMP:
			if (child->type != RZ_JSON_INTEGER) {
				break;
			}
			rz_analysis_hint_set_jump (anal, addr, child->num.u_value);
			break;
		case HINTS_FIELD_FAIL:
			if (child->type != RZ_JSON_INTEGER) {
				break;
			}
			rz_analysis_hint_set_fail (anal, addr, child->num.u_value);
			break;
		case HINTS_FIELD_STACKFRAME:
			if (child->type != RZ_JSON_INTEGER) {
				break;
			}
			rz_analysis_hint_set_stackframe (anal, addr, child->num.u_value);
			break;
		case HINTS_FIELD_PTR:
			if (child->type != RZ_JSON_INTEGER) {
				break;
			}
			rz_analysis_hint_set_pointer (anal, addr, child->num.u_value);
			break;
		case HINTS_FIELD_NWORD:
			if (child->type != RZ_JSON_INTEGER) {
				break;
			}
			rz_analysis_hint_set_nword (anal, addr, (int)child->num.s_value);
			break;
		case HINTS_FIELD_RET:
			if (child->type != RZ_JSON_INTEGER) {
				break;
			}
			rz_analysis_hint_set_ret (anal, addr, child->num.u_value);
			break;
		case HINTS_FIELD_NEW_BITS:
			if (child->type != RZ_JSON_INTEGER) {
				break;
			}
			rz_analysis_hint_set_newbits (anal, addr, (int)child->num.s_value);
			break;
		case HINTS_FIELD_SIZE:
			if (child->type != RZ_JSON_INTEGER) {
				break;
			}
			rz_analysis_hint_set_size (anal, addr, child->num.u_value);
			break;
		case HINTS_FIELD_SYNTAX:
			if (child->type != RZ_JSON_STRING) {
				break;
			}
			rz_analysis_hint_set_syntax (anal, addr, child->str_value);
			break;
		case HINTS_FIELD_OPTYPE:
			if (child->type != RZ_JSON_INTEGER) {
				break;
			}
			rz_analysis_hint_set_type (anal, addr, (int)child->num.s_value);
			break;
		case HINTS_FIELD_OPCODE:
			if (child->type != RZ_JSON_STRING) {
				break;
			}
			rz_analysis_hint_set_opcode (anal, addr, child->str_value);
			break;
		case HINTS_FIELD_TYPE_OFFSET:
			if (child->type != RZ_JSON_STRING) {
				break;
			}
			rz_analysis_hint_set_offset (anal, addr, child->str_value);
			break;
		case HINTS_FIELD_ESIL:
			if (child->type != RZ_JSON_STRING) {
				break;
			}
			rz_analysis_hint_set_esil (anal, addr, child->str_value);
			break;
		case HINTS_FIELD_HIGH:
			if (child->type != RZ_JSON_BOOLEAN || !child->num.u_value) {
				break;
			}
			rz_analysis_hint_set_high (anal, addr);
			break;
		case HINTS_FIELD_VAL:
			if (child->type != RZ_JSON_INTEGER) {
				break;
			}
			rz_analysis_hint_set_val (anal, addr, child->num.u_value);
			break;
		default:
			break;
	})
