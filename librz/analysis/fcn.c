...
// TODO: move into io :?
static int read_ahead(RzAnalysis *anal, ut64 addr, ut8 *buf, int len) {
...
static RzAnalysisBlock *fcn_append_basic_block(RzAnalysis *anal, RzAnalysisFunction *fcn, ut64 addr) {
	RzAnalysisBlock *bb = rz_analysis_create_block (anal, addr, 0);
	if (!bb) {
		return NULL;
	}
...
static bool isInvalidMemory(RzAnalysis *anal, const ut8 *buf, int len) {
...
static bool isSymbolNextInstruction(RzAnalysis *anal, RzAnalysisOp *op) {
...
static bool is_delta_pointer_table(RzAnalysis *anal, RzAnalysisFunction *fcn, ut64 addr, ut64 lea_ptr, ut64 *jmptbl_addr, ut64 *casetbl_addr, RzAnalysisOp *jmp_aop) {
	int i;
	ut64 dst;
	st32 jmptbl[64] = {0};
	...
	for (i = 0; i + 8 < JMPTBL_LEA_SEARCH_SZ; i++) {
		...
		int len = rz_analysis_op (anal, aop, at, buf + i, left, RZ_ANAL_OP_MASK_BASIC | RZ_ANAL_OP_MASK_HINT | RZ_ANAL_OP_MASK_VAL);
		if (len < 1) {
			len = 1;
		}
...
static ut64 try_get_cmpval_from_parents(RzAnalysis * anal, RzAnalysisFunction *fcn, RzAnalysisBlock *my_bb, const char * cmp_reg) {
...
static int skip_hp(RzAnalysis *anal, RzAnalysisFunction *fcn, RzAnalysisOp *op, RzAnalysisBlock *bb, ut64 addr,
...
static void check_purity(HtUP *ht, RzAnalysisFunction *fcn) {
	...
	rz_list_foreach (refs, iter, ref) {
		if (ref->type == RZ_ANAL_REF_TYPE_CALL || ref->type == RZ_ANAL_REF_TYPE_CODE) {
			RzAnalysisFunction *called_fcn = rz_analysis_get_fcn_in (fcn->anal, ref->addr, 0);
			if (!called_fcn) {
				continue;
			}
...
static RzAnalysisBlock *bbget(RzAnalysis *anal, ut64 addr, bool jumpmid) {
	RzList *intersecting = rz_analysis_get_blocks_in (anal, addr);
	RzListIter *iter;
	RzAnalysisBlock *bb;
			...
			if (anal->opt.delay) {
				...
				if (anal->iob.read_at (anal->iob.io, bb->addr, buf, bb->size)) {
					...
					for (i = last_instr_idx; i >= 0; i--) {
						...
						RzAnalysisOp op;
						int size = rz_analysis_op (anal, &op, at, buf + off, bb->size - off, RZ_ANAL_OP_MASK_BASIC);
						if (size > 0 && op.delay) {
...
static const char *retpoline_reg(RzAnalysis *anal, ut64 addr) {
...
static void analyze_retpoline(RzAnalysis *anal, RzAnalysisOp *op) {
...
static int fcn_recurse(RzAnalysis *anal, RzAnalysisFunction *fcn, ut64 addr, ut64 len, int depth) {
	...
	RzAnalysisFunction *fcn_at_addr = rz_analysis_get_function_at (anal, addr);
	if (fcn_at_addr && fcn_at_addr != fcn) {
		return RZ_ANAL_RET_ERROR; // MUST BE NOT FOUND
	}

	RzAnalysisBlock *existing_bb = bbget (anal, addr, anal->opt.jmpmid && is_x86);
	if (existing_bb) {
	...
	RzAnalysisFunction *tmp_fcn = rz_analysis_get_fcn_in (anal, addr, 0);
	if (tmp_fcn) {
		// Checks if var is already analyzed at given addr
		RzList *list = rz_analysis_var_all_list (anal, tmp_fcn);
...
repeat:
		...
		rz_analysis_op_fini (&op);
		if ((oplen = rz_analysis_op (anal, &op, at, buf, bytes_read, RZ_ANAL_OP_MASK_ESIL | RZ_ANAL_OP_MASK_VAL | RZ_ANAL_OP_MASK_HINT)) < 1) {
			if (anal->verbose) {
		...
		if (op.hint.new_bits) {
			rz_analysis_hint_set_bits (anal, op.jump, op.hint.new_bits);
		}
		if (idx > 0 && !overlapped) {
			...
			if (name) {
				if (rz_str_startswith (name, "try.") && rz_str_endswith (name, ".from")) {
					...
					if (from_addr != bb->addr) {
						bb->fail = handle_addr;
						ret = rz_analysis_fcn_bb (anal, fcn, handle_addr, depth);
		...
		if (op.ptr && op.ptr != UT64_MAX && op.ptr != UT32_MAX) {
			// swapped parameters
			rz_analysis_xrefs_set (anal, op.addr, op.ptr, RZ_ANAL_REF_TYPE_DATA);
		}
		...
		case RZ_ANAL_OP_TYPE_JMP:
			...
			if (anal->opt.jmpref) {
				(void) rz_analysis_xrefs_set (anal, op.addr, op.jump, RZ_ANAL_REF_TYPE_CODE);
			}
			if (!anal->opt.jmpabove && (op.jump < fcn->addr)) {
				gotoBeach (RZ_ANAL_RET_END);
			}
			if (rz_analysis_noreturn_at (anal, op.jump)) {
				gotoBeach (RZ_ANAL_RET_END);
			}
			{
				bool must_eob = true;
			...
			ret = rz_analysis_fcn_bb (anal, fcn, op.jump, depth);
			...
			if (tc) {
				...
				if (tc < 0) {
					...
					if (rz_analysis_is_prelude (anal, buf, sizeof (buf))) {
						fcn_recurse (anal, fcn, op.jump, anal->opt.bb_max_size, depth - 1);
					}
				} else if (RZ_ABS (diff) > tc) {
					(void) rz_analysis_xrefs_set (anal, op.addr, op.jump, RZ_ANAL_REF_TYPE_CALL);
					fcn_recurse (anal, fcn, op.jump, anal->opt.bb_max_size, depth - 1);
					gotoBeach (RZ_ANAL_RET_END);
				}
			}
		...
		case RZ_ANAL_OP_TYPE_UCJMP:
			...
			if (continue_after_jump) {
				rz_analysis_fcn_bb (anal, fcn, op.jump, depth);
				fcn->stack = saved_stack;
				ret = rz_analysis_fcn_bb (anal, fcn, op.fail, depth);
				fcn->stack = saved_stack;
			} else {
				ret = rz_analysis_fcn_bb (anal, fcn, op.jump, depth);
				fcn->stack = saved_stack;
		...
		case RZ_ANAL_OP_TYPE_IRCALL:
			...
			(void) rz_analysis_xrefs_set (anal, op.addr, op.ptr, RZ_ANAL_REF_TYPE_CALL);

			if (rz_analysis_noreturn_at (anal, op.ptr)) {
				RzAnalysisFunction *f = rz_analysis_get_function_at (anal, op.ptr);
				if (f) {
					f->is_noreturn = true;
				}
				gotoBeach (RZ_ANAL_RET_END);
		...
		case RZ_ANAL_OP_TYPE_CALL:
			...
			if (rz_analysis_noreturn_at (anal, op.jump)) {
				RzAnalysisFunction *f = rz_analysis_get_function_at (anal, op.jump);
				if (f) {
					f->is_noreturn = true;
				}
				gotoBeach (RZ_ANAL_RET_END);
		...
		case RZ_ANAL_OP_TYPE_IRJMP:
			...
			if (anal->opt.jmptbl && lea_jmptbl_ip != op.addr) {
				...
				if (op.ptr != UT64_MAX && op.ireg) { // direct jump
					...
					if (try_get_jmptbl_info (anal, fcn, op.addr, bb, &table_size, &default_case)) {
						...
						if (rz_analysis_op (anal, &prev_op, op.addr - op.size, buf, sizeof (buf), RZ_ANAL_OP_MASK_VAL) > 0) {
			...
			if (anal->opt.ijmp) {
				...
				if (rz_analysis_noreturn_at (anal, op.jump) || op.eob) {
					goto analopfinish;
				}
			} else {
analopfinish:
		...
		case RZ_ANAL_OP_TYPE_PUSH:
			...
			if (anal->iob.is_valid_offset (anal->iob.io, last_push_addr, 1)) {
				(void) rz_analysis_xrefs_set (anal, op.addr, last_push_addr, RZ_ANAL_REF_TYPE_DATA);
			}
			break;
		...
		if (anal->opt.vars && !varset) {
			rz_analysis_extract_vars (anal, fcn, &op);
		}
...
RZ_API bool rz_analysis_check_fcn(RzAnalysis *anal, ut8 *buf, ut16 bufsz, ut64 addr, ut64 low, ut64 high) {
	...
	if (rz_analysis_is_prelude (anal, buf, bufsz)) {
		return true;
	}
	for (i = 0; i < bufsz && opcnt < 10; i += oplen, opcnt++) {
		rz_analysis_op_fini (&op);
		if ((oplen = rz_analysis_op (anal, &op, addr + i, buf + i, bufsz - i, RZ_ANAL_OP_MASK_BASIC | RZ_ANAL_OP_MASK_HINT)) < 1) {
			return false;
		}
		switch (op.type) {
		case RZ_ANAL_OP_TYPE_PUSH:
...
RZ_API void rz_analysis_trim_jmprefs(RzAnalysis *anal, RzAnalysisFunction *fcn) {
	...
	rz_list_foreach (refs, iter, ref) {
		if (ref->type == RZ_ANAL_REF_TYPE_CODE && rz_analysis_function_contains (fcn, ref->addr)
		    && (!is_x86 || !rz_analysis_function_contains (fcn, ref->at))) {
			rz_analysis_xrefs_deln (anal, ref->at, ref->addr, ref->type);
		}
	}
	rz_list_free (refs);
}
...
RZ_API int rz_analysis_fcn(RzAnalysis *anal, RzAnalysisFunction *fcn, ut64 addr, ut64 len, int reftype) {
	...
	int ret = rz_analysis_fcn_bb (anal, fcn, addr, anal->opt.depth);
	if (ret < 0) {
		if (anal->verbose) {
			eprintf ("Failed to analyze basic block at 0x%"PFMT64x"\n", addr);
		}
	}
	if (anal->opt.endsize && ret == RZ_ANAL_RET_END && rz_analysis_function_realsize (fcn)) {   // cfg analysis completed
		RzListIter *iter;
		RzAnalysisBlock *bb;
...
#endif
		rz_analysis_trim_jmprefs (anal, fcn);
	}
	return ret;
}
...
RZ_API int rz_analysis_fcn_del_locs(RzAnalysis *anal, ut64 addr) {
	RzListIter *iter, *iter2;
	RzAnalysisFunction *fcn, *f = rz_analysis_get_fcn_in (anal, addr, RZ_ANAL_FCN_TYPE_ROOT);
	if (!f) {
		return false;
	}
...
RZ_API RzAnalysisFunction *rz_analysis_get_fcn_in(RzAnalysis *anal, ut64 addr, int type) {
	RzList *list = rz_analysis_get_functions_in (anal, addr);
...
RZ_API RzAnalysisFunction *rz_analysis_get_fcn_in_bounds(RzAnalysis *anal, ut64 addr, int type) {
...
RZ_API int rz_analysis_fcn_count(RzAnalysis *anal, ut64 from, ut64 to) {
	int n = 0;
	RzAnalysisFunction *fcni;
	RzListIter *iter;
	rz_list_foreach (anal->fcns, iter, fcni) {
...
RZ_API RzAnalysisBlock *rz_analysis_fcn_bbget_in(const RzAnalysis *anal, RzAnalysisFunction *fcn, ut64 addr) {
	...
	rz_list_foreach (fcn->bbs, iter, bb) {
		if (addr >= bb->addr && addr < (bb->addr + bb->size)
			&& (!anal->opt.jmpmid || !is_x86 || rz_analysis_block_op_starts_at (bb, addr))) {
			return bb;
		}
	}
	return NULL;
...
RZ_API RzAnalysisBlock *rz_analysis_fcn_bbget_at(RzAnalysis *anal, RzAnalysisFunction *fcn, ut64 addr) {
	...
	RzAnalysisBlock *b = rz_analysis_get_block_at (anal, addr);
	if (b) {
		return b;
	}
	RzListIter *iter;
...
RZ_API ut32 rz_analysis_function_cost(RzAnalysisFunction *fcn) {
	...
	rz_list_foreach (fcn->bbs, iter, bb) {
		...
		for (at = bb->addr; at < end;) {
			...
			(void) rz_analysis_op (anal, &op, at, buf + idx, bb->size - idx, RZ_ANAL_OP_MASK_BASIC);
			if (op.size < 1) {
				op.size = 1;
			}
			idx += op.size;
...
static bool can_affect_bp(RzAnalysis *anal, RzAnalysisOp* op) {
 ...
 */
static void __anal_fcn_check_bp_use(RzAnalysis *anal, RzAnalysisFunction *fcn) {
	RzListIter *iter;
	RzAnalysisBlock *bb;
	...
	rz_list_foreach (fcn->bbs, iter, bb) {
		...
		int idx = 0;
		for (at = bb->addr; at < end;) {
			rz_analysis_op (anal, &op, at, buf + idx, bb->size - idx, RZ_ANAL_OP_MASK_VAL | RZ_ANAL_OP_MASK_OPEX);
			if (op.size < 1) {
				op.size = 1;
			}
			switch (op.type) {
...
static bool analize_addr_cb(ut64 addr, void *user) {
	BlockRecurseCtx *ctx = user;
	RzAnalysis *anal = ctx->fcn->anal;
	RzAnalysisBlock *existing_bb = rz_analysis_get_block_at (anal, addr);
	if (!existing_bb || !rz_list_contains (ctx->fcn->bbs, existing_bb)) {
		int old_len = rz_list_length (ctx->fcn->bbs);
		rz_analysis_fcn_bb (ctx->fcn->anal, ctx->fcn, addr, anal->opt.depth);
		if (old_len != rz_list_length (ctx->fcn->bbs)) {
			rz_analysis_block_recurse (rz_analysis_get_block_at (anal, addr), mark_as_visited, user);
		}
	}
...
static void update_varz_analysisysis(RzAnalysisFunction *fcn, int align, ut64 from, ut64 to) {
	...
	for (cur_addr = from; cur_addr < to; cur_addr += opsz, len -= opsz) {
		RzAnalysisOp op;
		int ret = rz_analysis_op (anal->coreb.core, &op, cur_addr, buf, len, RZ_ANAL_OP_MASK_ESIL | RZ_ANAL_OP_MASK_VAL);
		if (ret < 1 || op.size < 1) {
			rz_analysis_op_fini (&op);
			break;
		}
...
static void update_analysis(RzAnalysis *anal, RzList *fcns, HtUP *reachable) {
	...
	rz_list_foreach (fcns, it, fcn) {
		...
		RzAnalysisBlock *bb = rz_analysis_get_block_at (anal, fcn->addr);
		if (!bb) {
			rz_analysis_fcn_bb (anal, fcn, fcn->addr, anal->opt.depth);
			bb = rz_analysis_get_block_at (anal, fcn->addr);
			if (!bb) {
				continue;
			}
		}
...
static void calc_reachable_and_remove_block(RzList *fcns, RzAnalysisFunction *fcn, RzAnalysisBlock *bb, HtUP *reachable) {
	...
	if (!rz_list_contains (fcns, fcn)) {
		...
		HtUP *ht = ht_up_new0 ();
		BlockRecurseCtx ctx = { fcn, ht };
		rz_analysis_block_recurse (rz_analysis_get_block_at (fcn->anal, fcn->addr), mark_as_visited, &ctx);
...
RZ_API void rz_analysis_update_analysis_range(RzAnalysis *anal, ut64 addr, int size) {
	...
	RzAnalysisBlock *bb;
	RzAnalysisFunction *fcn;
	RzList *blocks = rz_analysis_get_blocks_intersect (anal, addr, size);
	...
	const int align = rz_analysis_archinfo (anal, RZ_ANAL_ARCHINFO_ALIGN);
