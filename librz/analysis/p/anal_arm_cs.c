...
static int cond_cs2r2(int cc) {
	if (cc == ARM_CC_AL || cc < 0) {
		...
		switch (cc) {
		case ARM_CC_EQ: cc = RZ_ANAL_COND_EQ; break;
		case ARM_CC_NE: cc = RZ_ANAL_COND_NE; break;
		case ARM_CC_HS: cc = RZ_ANAL_COND_HS; break;
		case ARM_CC_LO: cc = RZ_ANAL_COND_LO; break;
		case ARM_CC_MI: cc = RZ_ANAL_COND_MI; break;
		case ARM_CC_PL: cc = RZ_ANAL_COND_PL; break;
		case ARM_CC_VS: cc = RZ_ANAL_COND_VS; break;
		case ARM_CC_VC: cc = RZ_ANAL_COND_VC; break;
		case ARM_CC_HI: cc = RZ_ANAL_COND_HI; break;
		case ARM_CC_LS: cc = RZ_ANAL_COND_LS; break;
		case ARM_CC_GE: cc = RZ_ANAL_COND_GE; break;
		case ARM_CC_LT: cc = RZ_ANAL_COND_LT; break;
		case ARM_CC_GT: cc = RZ_ANAL_COND_GT; break;
		case ARM_CC_LE: cc = RZ_ANAL_COND_LE; break;
		}
	}
	return cc;
}
...
static void anop64(csh handle, RzAnalysisOp *op, cs_insn *insn) {
	...
	case ARM64_INS_XPACLRI:
		...
		op->family = RZ_ANAL_OP_FAMILY_SECURITY;
		break;
#endif
	case ARM64_INS_SVC:
	...
	case ARM64_INS_SUB:
		if (ISREG64(0) && REGID64(0) == ARM64_REG_SP) {
			...
			op->stackop = RZ_ANAL_STACK_RESET;
			op->stackptr = 0;
		}
		op->cycles = 1;
		/* fallthru */
	...
	case ARM64_INS_DC: // data cache invalidate
		op->type = RZ_ANAL_OP_TYPE_SYNC; // or cache
		break;
	...
	case ARM64_INS_TBNZ:
		...
		break;
	case ARM64_INS_BR:
		op->type = RZ_ANAL_OP_TYPE_UJMP; // RJMP ?
		op->eob = true;
		break;
	case ARM64_INS_B:
		// BX LR == RET
...
static void anop32(RzAnalysis *a, csh handle, RzAnalysisOp *op, cs_insn *insn, bool thumb, const ut8 *buf, int len) {
	...
	if (cs_insn_group (handle, insn, ARM_GRP_CRYPTO)) {
		...
		op->family = RZ_ANAL_OP_FAMILY_VIRT;
#endif
...
jmp $$ + 4 + ( [delta] * 2 )
	...
	case ARM_INS_TBB: // byte jump table
		...
		break;
	case ARM_INS_PLD:
		op->type = RZ_ANAL_OP_TYPE_LEA; // not really a lea, just a prefetch
		if (ISMEM (0)) {
...
static ut8 *anal_mask(RzAnalysis *anal, int size, const ut8 *data, ut64 at) {
	...
	while (idx < size) {
		hint = rz_analysis_hint_get (anal, at + idx);
		if (hint) {
			if (hint->bits != 0) {
				anal->bits = hint->bits;
			}
		...
		if ((oplen = analop (anal, op, at + idx, data + idx, size - idx, RZ_ANAL_OP_MASK_BASIC)) < 1) {
			break;
		}
		if (op->ptr != UT64_MAX || op->jump != UT64_MAX) {
			...
			ut32 opcode = rz_read_ble (data + idx, anal->big_endian, oplen * 8);
			switch (oplen) {
			case 2:
			...
			case 4:
				if (anal->bits == 64) {
					...
					case ARM64_INS_STXRB:
					case ARM64_INS_STXRH:
						rz_write_ble (ret + idx, 0xffffffff, anal->big_endian, 32);
						break;
					case ARM64_INS_STRB:
					case ARM64_INS_STURB:
					case ARM64_INS_STURH:
					...
					case ARM64_INS_LDRSH: {
						...
						if (is_literal) {
							rz_write_ble (ret + idx, 0xff000000, anal->big_endian, 32);
						} else {
							rz_write_ble (ret + idx, 0xffffffff, anal->big_endian, 32);
						}
						break;
					...
					case ARM64_INS_CBNZ:
						if (op->type == RZ_ANAL_OP_TYPE_CJMP) {
							rz_write_ble (ret + idx, 0xff00001f, anal->big_endian, 32);
						} else {
							rz_write_ble (ret + idx, 0xfc000000, anal->big_endian, 32);
						}
						break;
					case ARM64_INS_TBZ:
					case ARM64_INS_TBNZ:
						rz_write_ble (ret + idx, 0xfff8001f, anal->big_endian, 32);
						break;
					case ARM64_INS_ADR:
					case ARM64_INS_ADRP:
						rz_write_ble (ret + idx, 0xff00001f, anal->big_endian, 32);
						break;
					default:
						rz_write_ble (ret + idx, 0xfff00000, anal->big_endian, 32);
					}
				} else {
					rz_write_ble (ret + idx, 0xfff00000, anal->big_endian, 32);
				}
