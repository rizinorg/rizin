...
static int wasm_op(RzAnalysis *anal, RzAnalysisOp *op, ut64 addr, const ut8 *data, int len, RzAnalysisOpMask mask) {
	...
	case WASM_TYPE_OP_CORE:
		...
		case WASM_OP_LOOP:
			...
			if (!(hint = rz_analysis_hint_get (anal, addr))) {
				scope_hint--;
				rz_analysis_hint_set_opcode (anal, scope_hint, "loop");
				rz_analysis_hint_set_jump (anal, scope_hint, addr);
			}
			break;
		case WASM_OP_BLOCK:
			op->type = RZ_ANAL_OP_TYPE_NOP;
			if (!(hint = rz_analysis_hint_get (anal, addr))) {
				scope_hint--;
				rz_analysis_hint_set_opcode (anal, scope_hint, "block");
				rz_analysis_hint_set_jump (anal, scope_hint, addr);
			}
			break;
		case WASM_OP_IF:
			if (!(hint = rz_analysis_hint_get (anal, addr))) {
				scope_hint--;
				rz_analysis_hint_set_opcode (anal, scope_hint, "if");
				rz_analysis_hint_set_jump (anal, scope_hint, addr);
		...
		case WASM_OP_BR:
			{
				...
				if ((hint2 = rz_analysis_hint_get (anal, addr)) && hint2->jump != UT64_MAX) {
					op->type = RZ_ANAL_OP_TYPE_JMP;
					op->jump = hint2->jump;
				} else if ((hint = rz_analysis_hint_get (anal, scope_hint))) {
					if (hint->opcode && !strncmp ("loop", hint->opcode, 4)) {
						op->type = RZ_ANAL_OP_TYPE_JMP;
						op->jump = hint->jump;
						rz_analysis_hint_set_jump (anal, addr, op->jump);
					} else {
		...
		case WASM_OP_END:
			{
				...
				if (scope_hint < UT64_MAX) {
					hint = rz_analysis_hint_get (anal, scope_hint);
					if (hint && !strncmp ("loop", hint->opcode, 4)) {
						rz_analysis_hint_set_jump (anal, addr, op->jump);
						rz_analysis_hint_set_jump (anal, op->jump, addr);
					} else if (hint && !strncmp ("block", hint->opcode, 5)) {
						// if/else/block
						rz_analysis_hint_set_jump (anal, hint->jump, addr);
						rz_analysis_hint_set_jump (anal, addr, UT64_MAX);
					}
					if (hint) {
						rz_analysis_hint_set_opcode (anal, scope_hint, "invalid");
						rz_analysis_hint_set_jump (anal, scope_hint, UT64_MAX);
						rz_analysis_hint_del (anal, scope_hint, 1);
						scope_hint++;
					} else {
