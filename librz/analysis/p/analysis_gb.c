...
static inline void gb_analysis_cb_rlc(RzAnalysisOpMask mask, RzReg *reg, RzAnalysisOp *op, const ut8 data, bool set_z) {
	op->dst = rz_analysis_value_new();
	op->src[0] = rz_analysis_value_new();
	op->src[0]->imm = 1;
...
static int gb_anop(RzAnalysis *analysis, RzAnalysisOp *op, ut64 addr, const ut8 *data, int len, RzAnalysisOpMask mask) {
	...
	case 0x07: // rlca
		...
		gb_analysis_cb_rlc(mask, analysis->reg, op, 7, false);
		break;
	case 0x17: // rla
		op->cycles = 4;
		op->type = RZ_ANALYSIS_OP_TYPE_ROL;
	...
	case 0xcb:
		...
		case 0:
			...
			op->type = RZ_ANALYSIS_OP_TYPE_ROL;
			gb_analysis_cb_rlc(mask, analysis->reg, op, data[1], true);
			break;
		case 1:
			if ((data[1] & 7) == 6) {
				op->cycles = 16;
