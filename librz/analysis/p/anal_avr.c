...
INST_HANDLER (eijmp) {	// EIJMP
	...
	rz_analysis_esil_reg_read (anal->esil, "z",    &z,    NULL);
	rz_analysis_esil_reg_read (anal->esil, "eind", &eind, NULL);
...
INST_HANDLER (spm) {	// SPM Z+
	...
	rz_analysis_esil_reg_read (anal->esil, "spmcsr", &spmcsr, NULL);
...
static int avr_op(RzAnalysis *anal, RzAnalysisOp *op, ut64 addr, const ut8 *buf, int len, RzAnalysisOpMask mask) {
	...
	if (anal->esil) {
		offset = 0;
		rz_analysis_esil_reg_write (anal->esil, "_prog", offset);

		offset += (1 << cpu->pc);
		rz_analysis_esil_reg_write (anal->esil, "_io", offset);

		offset += const_get_value (const_by_name (cpu, CPU_CONST_PARAM, "sram_start"));
		rz_analysis_esil_reg_write (anal->esil, "_sram", offset);

		offset += const_get_value (const_by_name (cpu, CPU_CONST_PARAM, "sram_size"));
		rz_analysis_esil_reg_write (anal->esil, "_eeprom", offset);

		offset += const_get_value (const_by_name (cpu, CPU_CONST_PARAM, "eeprom_size"));
		rz_analysis_esil_reg_write (anal->esil, "_page", offset);
	}
	// process opcode
...
static int esil_avr_init(RzAnalysisEsil *esil) {
	...
	desctx.round = 0;
	rz_analysis_esil_set_op (esil, "des", avr_custom_des, 0, 0, RZ_ANAL_ESIL_OP_TYPE_CUSTOM);		//better meta info plz
	rz_analysis_esil_set_op (esil, "SPM_PAGE_ERASE", avr_custom_spm_page_erase, 0, 0, RZ_ANAL_ESIL_OP_TYPE_CUSTOM);
	rz_analysis_esil_set_op (esil, "SPM_PAGE_FILL", avr_custom_spm_page_fill, 0, 0, RZ_ANAL_ESIL_OP_TYPE_CUSTOM);
	rz_analysis_esil_set_op (esil, "SPM_PAGE_WRITE", avr_custom_spm_page_write, 0, 0, RZ_ANAL_ESIL_OP_TYPE_CUSTOM);
