...
#define GRUB_PRzIO_LIST_PRzIO_MASK	0xff
#define GRUB_PRzIO_LIST_FLAG_ACTIVE	0x100
...
{
  if ((item->prio & GRUB_PRzIO_LIST_FLAG_ACTIVE) && (item->next))
    item->next->prio |= GRUB_PRzIO_LIST_FLAG_ACTIVE;
  grub_list_remove (GRUB_AS_LIST_P (head), GRUB_AS_LIST (item));
}

#define GRUB_AS_PRzIO_LIST(ptr) \
...
#define GRUB_AS_PRzIO_LIST_P(pptr) \
