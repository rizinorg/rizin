...
bool test_meta_set() {
	...
	rz_meta_set (anal, RZ_META_TYPE_DATA, 0x100, 8, NULL);
	...
	rz_meta_set_string (anal, RZ_META_TYPE_COMMENT, 0x100, "this ain't the summer of love");
	rz_meta_set_with_subtype (anal, RZ_META_TYPE_STRING, RZ_STRING_ENC_UTF16LE, 0x200, 0x40, "e.t.i. (extra terrestrial intelligence)");
...
bool test_meta_get_at() {
	...
	ut64 size;
	item = rz_meta_get_at (anal, 0x100, RZ_META_TYPE_DATA, &size);
	mu_assert_notnull (item, "get item");
	mu_assert_eq (item->type, RZ_META_TYPE_DATA, "get contents");
	mu_assert_eq (size, 4, "get size");

	item = rz_meta_get_at (anal, 0x200, RZ_META_TYPE_ANY, NULL);
	mu_assert_notnull (item, "get item");
	mu_assert_streq (item->str, "true confessions", "get contents");

	item = rz_meta_get_at (anal, 0x100, RZ_META_TYPE_ANY, NULL);
	mu_assert_notnull (item, "get item");
	// which one we get is undefined here (intended)

	item = rz_meta_get_at (anal, 0x1ff, RZ_META_TYPE_ANY, NULL);
	mu_assert_null (item, "get item");
	item = rz_meta_get_at (anal, 0x201, RZ_META_TYPE_ANY, NULL);
	mu_assert_null (item, "get item");
	item = rz_meta_get_at (anal, 0xff, RZ_META_TYPE_ANY, NULL);
	mu_assert_null (item, "get item");
...
bool test_meta_get_in() {
	...
	node = rz_meta_get_in (anal, 0x101, RZ_META_TYPE_COMMENT);
	mu_assert_null (node, "get item");
	...
	item = node->data;
	mu_assert_eq (item->type, RZ_META_TYPE_DATA, "get contents");
	node = rz_meta_get_in (anal, 0xff, RZ_META_TYPE_DATA);
	mu_assert_null (node, "get item");
	node = rz_meta_get_in (anal, 0x103, RZ_META_TYPE_DATA);
	mu_assert_notnull (node, "get item");
	item = node->data;
	mu_assert_eq (item->type, RZ_META_TYPE_DATA, "get contents");
	node = rz_meta_get_in (anal, 0x104, RZ_META_TYPE_DATA);
	mu_assert_null (node, "get item");

	node = rz_meta_get_in (anal, 0x103, RZ_META_TYPE_ANY);
	mu_assert_notnull (node, "get item");
	item = node->data;
	mu_assert_eq (item->type, RZ_META_TYPE_DATA, "get contents");

	node = rz_meta_get_in (anal, 0x100, RZ_META_TYPE_ANY);
	mu_assert_notnull (node, "get item");
	// which one we get is undefined here (intended)
...
bool test_meta_get_all_in() {
	...
	items = rz_meta_get_all_in (anal, 0xff, RZ_META_TYPE_ANY);
	mu_assert_eq (rz_pvector_len (items), 0, "all count");
	rz_pvector_free (items);

	items = rz_meta_get_all_in (anal, 0x101, RZ_META_TYPE_COMMENT);
	mu_assert_eq (rz_pvector_len (items), 0, "all count");
	rz_pvector_free (items);
	...
	items = rz_meta_get_all_in (anal, 0x104, RZ_META_TYPE_DATA);
	mu_assert_eq (rz_pvector_len (items), 0, "all count");
	rz_pvector_free (items);
...
bool test_meta_get_all_intersect() {
	...
	items = rz_meta_get_all_intersect (anal, 0x0, 0x100, RZ_META_TYPE_DATA);
	mu_assert_eq (rz_pvector_len (items), 0, "all count");
	rz_pvector_free (items);
	...
	items = rz_meta_get_all_intersect (anal, 0x104, 0x300, RZ_META_TYPE_DATA);
	mu_assert_eq (rz_pvector_len (items), 0, "all count");
	rz_pvector_free (items);
...
bool test_meta_del() {
	...
	rz_meta_del (anal, RZ_META_TYPE_COMMENT, 0x0, 0x500);
	item = rz_meta_get_at (anal, 0x100, RZ_META_TYPE_COMMENT, NULL);
	mu_assert_null (item, "item deleted");
	item = rz_meta_get_at (anal, 0x100, RZ_META_TYPE_DATA, NULL);
	mu_assert_notnull (item, "item not deleted");
	item = rz_meta_get_at (anal, 0x200, RZ_META_TYPE_STRING, NULL);
	...
	rz_meta_del (anal, RZ_META_TYPE_COMMENT, 0, UT64_MAX);
	item = rz_meta_get_at (anal, 0x100, RZ_META_TYPE_COMMENT, NULL);
	mu_assert_null (item, "item deleted");
	item = rz_meta_get_at (anal, 0x100, RZ_META_TYPE_DATA, NULL);
	mu_assert_notnull (item, "item not deleted");
	...
	rz_meta_del (anal, RZ_META_TYPE_ANY, 0, 0x500);
	item = rz_meta_get_at (anal, 0x100, RZ_META_TYPE_COMMENT, NULL);
	mu_assert_null (item, "item deleted");
	item = rz_meta_get_at (anal, 0x100, RZ_META_TYPE_DATA, NULL);
	mu_assert_null (item, "item deleted");
	...
	rz_meta_del (anal, RZ_META_TYPE_ANY, 0, UT64_MAX);
	item = rz_meta_get_at (anal, 0x100, RZ_META_TYPE_COMMENT, NULL);
	mu_assert_null (item, "item deleted");
	item = rz_meta_get_at (anal, 0x100, RZ_META_TYPE_DATA, NULL);
	mu_assert_null (item, "item deleted");
...
bool test_meta_rebase() {
	...
	rz_meta_set_with_subtype (anal, RZ_META_TYPE_STRING, RZ_STRING_ENC_UTF8, 0x300, 0x30, "true confessions");
...
bool test_meta_spaces() {
	...
	rz_meta_set_string (anal, RZ_META_TYPE_COMMENT, 0x100, "reaper");
