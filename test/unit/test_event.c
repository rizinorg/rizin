...
bool test_r_event(void) {
	...
	REventCallbackHandle handle_all = rz_event_hook (ev, R_EVENT_ALL, callback_test, &acc_all);
	REventCallbackHandle handle_specific = rz_event_hook (ev, R_EVENT_META_SET, callback_test, &acc_specific);

	rz_event_send (ev, R_EVENT_META_DEL, (void *)0x4242);
	...
	rz_event_send (ev, R_EVENT_META_SET, (void *)0xdeadbeef);
	...
	rz_event_send (ev, R_EVENT_META_SET, (void *)0xc0ffee);
