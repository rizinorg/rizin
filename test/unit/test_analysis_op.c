...
bool test_rz_analysis_op_val() {
	...
	len = rz_analysis_op(analysis, &op, 0, "\x48\xc7\xc0\x04\x00\x00\x00", 7, RZ_ANALYSIS_OP_MASK_VAL);
	...
	// ldr x1, [x2, x3]
	len = rz_analysis_op(analysis, &op, 0, "\x41\x68\x63\xf8", 4, RZ_ANALYSIS_OP_MASK_VAL);
	...
	rz_analysis_op_fini(&op);
	// mov x1, 400
	len = rz_analysis_op(analysis, &op, 0, "\x01\x32\x80\xd2", 4, RZ_ANALYSIS_OP_MASK_VAL);
	mu_assert_eq(len, 4, "Op is of size 4");
	mu_assert_eq(op.dst->type, RZ_ANALYSIS_VAL_REG, "Destination should be reg");
	mu_assert_streq(op.dst->reg->name, "x1", "Dst reg should be x1");
	mu_assert_eq(op.src[0]->type, RZ_ANALYSIS_VAL_IMM, "Source should be imm");
	...
	// ldr r1, [ r2, r3 ]
	len = rz_analysis_op(analysis, &op, 0, "\x03\x10\x92\xe7", 4, RZ_ANALYSIS_OP_MASK_VAL);
	mu_assert_eq(len, 4, "Op is of size 4");
	mu_assert_eq(op.dst->type, RZ_ANALYSIS_VAL_REG, "Destination should be reg");
	...
	// ldr r1, [ r2, r3 ]
	len = rz_analysis_op(analysis, &op, 0, "\xd1\x58", 2, RZ_ANALYSIS_OP_MASK_VAL);
	...
	// lw s10, 64(sp)
	len = rz_analysis_op(analysis, &op, 0, "\x06\x4d", 2, RZ_ANALYSIS_OP_MASK_VAL);
	mu_assert_eq(len, 2, "Op is of size 2");
	mu_assert_eq(op.dst->type, RZ_ANALYSIS_VAL_REG, "Destination should be reg");
	...
	rz_analysis_op_fini(&op);
	// sw s0, 136(sp)
	len = rz_analysis_op(analysis, &op, 0, "\x22\xc5", 2, RZ_ANALYSIS_OP_MASK_VAL);
	mu_assert_eq(len, 2, "Op is of size 2");
