...
static bool test_migrate_v25_v26_analysis_prelude() {
	RzProject *prj = rz_project_load_file_raw("prj/v25-analysis-prelude.rzdb");
	mu_assert_notnull(prj, "load raw project");
	RzSerializeResultInfo *res = rz_serialize_result_info_new();
	...
	Sdb *config_db = sdb_ns(core_db, "config", false);
	mu_assert_notnull(config_db, "config ns");
	mu_assert_notnull(sdb_get(config_db, "analysis.prelude"), "analysis.prelude exists before migration");

	bool s = rz_project_migrate_v25_v26(prj, res);
	mu_assert_true(s, "migrate success");

	mu_assert_null(sdb_get(config_db, "analysis.prelude"), "analysis.prelude doesn't exist after migration");
...
int all_tests() {
	...
	mu_run_test(test_migrate_v25_v26_analysis_prelude);
