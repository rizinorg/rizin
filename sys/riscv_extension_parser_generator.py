...
class Trie:
    ...
    def generate_ascii_tree(self, node: Optional[TrieNode] = None, 
                           prefix: str = "", is_last: bool = True, 
                           char: str = "ROOT") -> List[str]:
        ...
        if node.children:
            ...
            for i, (ch, child) in enumerate(sorted_children):
                ...
                lines.extend(self.generate_ascii_tree(
                    child, child_prefix, is_last_child, ch
                ))
...
def generate_c_code(trie: Trie, extensions: List[Tuple[str, str]]) -> str:
    ...
    full_code += " * Generated from arch/RISCV/RISCVGenSubtargetInfo.inc in Capstone sources \n"
    full_code += " * \n"
    ...
    flags_defs = "\n".join([f"#define {member} {i} \n" for i, (_, member) in enumerate(extensions)])
    full_code += flags_defs

    full_code += "/* Parse result enum - indicates why parsing stopped */\n"
    full_code += "typedef enum {\n"
    full_code += "    RISCV_EXT_PARSE_END_OF_STRING,           /* Stopped at '\\0' - valid extension found */\n"
    full_code += "    RISCV_EXT_PARSE_STOPPED_AT_NUMBER,       /* Stopped at a digit - valid extension found */\n"
    full_code += "    RISCV_EXT_PARSE_STOPPED_AT_UNDERSCORE,   /* Stopped at '_' - valid extension found */\n"
    full_code += "    RISCV_EXT_PARSE_STOPPED_AT_UNEXPECTED,   /* Stopped at unexpected char - valid extension found */\n"
    full_code += "    RISCV_EXT_PARSE_NO_MATCH_END_OF_STRING,  /* Reached '\\0' - no valid extension */\n"
    full_code += "    RISCV_EXT_PARSE_NO_MATCH_NUMBER,         /* Hit number - no valid extension at this point */\n"
    full_code += "    RISCV_EXT_PARSE_NO_MATCH_UNDERSCORE,     /* Hit underscore - no valid extension at this point */\n"
    full_code += "    RISCV_EXT_PARSE_NO_MATCH_UNEXPECTED,     /* Hit unexpected char - no valid extension */\n"
    ...
    full_code += "/**\n"
    full_code += " * Main parse routine: Try to consume a RISC-V extension name from the architecture string at a given position.\n"
    full_code += " * \n"
    full_code += " * @param arch_str: The full architecture string\n"
    full_code += " * @param idx: Pointer to current index (will be updated to show consumed length)\n"
    full_code += " * @param mode: Pointer to the feature accumulator (will be ORed with the feature flag if extension found)\n"
    ...
    full_code += "try_consume_riscv_ext_from(const char *ext_name, size_t *idx, ut64 *mode)\n"
    full_code += "{\n"
...
def main():
    ...
    with open(input_header, 'r') as f:
        enum_text = "\n".join([line for line in f if line.strip().startswith('RISCV_FeatureStdExt')])
